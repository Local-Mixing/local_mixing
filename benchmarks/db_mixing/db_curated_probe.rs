...
fn main() {
    ...
    let path = a
        .next()
        .expect("usage: db_curated_probe <circuit> [samples] [max_window]");
    ...
    println!(
        "circuit {path}: {} gates, {num_wires} wires; {samples} windows up to {wmax}",
        gates.len()
    );
    ...
    println!(
        "  with any REGULAR candidate:        {w_reg} ({:.1}%)",
        pct(w_reg)
    );
    println!(
        "  with any CURATED FORWARD candidate:{w_cur_fwd} ({:.1}%)",
        pct(w_cur_fwd)
    );
    println!(
        "  with any CURATED REVERSE candidate:{w_cur_rev} ({:.1}%)",
        pct(w_cur_rev)
    );
