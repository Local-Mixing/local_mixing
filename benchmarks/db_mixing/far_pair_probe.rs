...
fn main() {
    ...
    for _ in 0..args.samples {
        ...
        if go_right {
            ...
            let mut j = i;
            while j > 0
                && hops_scanned < args.scan_cap
                && !XGate::collides(&gates[i], &gates[j - 1])
            {
                cands.push(j - 1);
                j -= 1;
                hops_scanned += 1;
    ...
    let hist: Vec<String> = len_hist
        ...
        .collect();
    println!(
        "[far_pair_probe] candidate gate-count histogram: {}",
        hist.join(" ")
    );
}
