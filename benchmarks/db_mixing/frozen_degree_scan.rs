...
fn main() {
            ...
            *span_hist.entry((gates.len(), min_span)).or_insert(0) += 1;
            match xgates_to_polynomial(&gates, nw, budget) {
                Ok(polys) => {
                    ...
                    *terms_hist
                        .entry(wire_terms.next_power_of_two())
                        .or_insert(0) += 1;
                    if min_span >= 24 {
                        wide_n += 1;
    ...
    println!("entries analyzed: {kept} (budget_fail {budget_fail}, parse_fail {parse_fail})");
    {
        ...
        for sp in 20..=smax {
            let row: Vec<u64> = (0..=jdmax)
                .map(|d| *joint.get(&(sp, d)).unwrap_or(&0))
                .collect();
        ...
        for (d, v) in &wide {
            println!(
                "   deg {d}: {v}  ({:.1}%)",
                100.0 * *v as f64 / wtot.max(1) as f64
            );
        }
        println!(
            "span >= 24 worst poly size: max per-wire terms {wide_terms_max}, max total terms {wide_total_max} (n={wide_n})"
        );
    }
