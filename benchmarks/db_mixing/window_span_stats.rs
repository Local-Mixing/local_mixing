...
use local_mixing::db_mixing::replace::{
    Tag, random_subcircuit, read_tags_sidecar, window_litter_stats,
};
...
fn sample<'a>(
    ...
    if local {
        ...
        let slice = CircuitSeq {
            gates: c.gates[off..off + 50].to_vec(),
        };
...
fn main() {
    ...
    let path = args
        .next()
        .expect("usage: window_span_stats <circuit.txt> [span_samples] [db_probes]");
    ...
    for &local in &[false, true] {
        ...
        for _ in 0..span_samples {
            let Some((sub, st, en)) = sample(&c, &tags, local, &mut rng) else {
                continue;
            };
