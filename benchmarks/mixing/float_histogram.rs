...
use local_mixing::db_mixing::replace::{Tag, float_all_gates};
use std::env;
use std::fs;
...
fn main() {
    let path = env::args()
        .nth(1)
        .expect("usage: float_histogram <circuit.txt> [passes]");
...
fn run_pass(c: &mut CircuitSeq, n: usize, pass: usize, rng: &mut impl rand::Rng) {
    ...
    println!(
        "SUMMARY pass={} gates={} in_pass_moves={} in_pass_summed_steps={}",
        pass, n, pass_moves, pass_disp
    );
    println!(
