...
fn main() {
    ...
    let path = a
        .next()
        .expect("usage: ladder_mobility <circuit.mpmct1> [sample] [cap] [seed]");
    ...
    let n = gates.len();
    eprintln!(
        "[ladmob] {} gates, {} wires, cap {}/side, sample {}",
        n, wires, cap, sample
    );
    ...
    println!(
        "[ladmob] gates={} T-bracket groups={} ({:.2}% of gates in a group)",
        n,
        groups.len(),
    ...
    for &gi in &idx {
        ...
        let chain = m
            .windows(2)
            .all(|w| XGate::collides(&gates[w[0]], &gates[w[1]]));
        if chain {
            frozen_chain += 1;
        }
    ...
    if !rig.is_empty() {
        ...
        sizes.sort_unstable();
        println!(
            "[ladmob] sampled {} groups  size mean={:.2} med={}",
            rig.len(),
            mean(&sizes),
            pct(&sizes, 0.5)
        );
        ...
        println!(
            ...
            frozen_chain,
            rig.len(),
            frozen_chain as f64 / rig.len() as f64
        );
        if n4 > 0 {
            println!(
    ...
    println!(
        "[ladmob] --- {} sweeps of the tagged commuting shuffle ---",
        sweeps
    );
    ...
    println!(
        ...
        order_kept,
        idx.len(),
        order_kept as f64 / idx.len() as f64
    );
}
