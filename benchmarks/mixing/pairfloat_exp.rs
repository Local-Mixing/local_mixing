...
impl WireIndex {
    ...
    fn gap(list: &[u32], i: usize) -> usize {
        ...
        let dl = if k == 0 {
            ...
            (i - list[k - 1] as usize).min(SCAN_CAP)
        };
        let dr = if k == list.len() {
            SCAN_CAP
        } else {
            (list[k] as usize - i + 1).min(SCAN_CAP)
        };
        dl + dr
    }
}
...
fn adaptive_conj_xgate(idx: &WireIndex, n: usize, i: usize, used_targets: &[u16]) -> XGate {
    ...
    XGate::conj(
        t,
        [
            (c_scores[a].1, fastrand::bool()),
            (c_scores[b].1, fastrand::bool()),
        ],
    )
    .expect("distinct wires cannot contradict")
}
...
fn main() {
    ...
    let out = a
        .next()
        .expect("usage: pairfloat_exp <outbase> [n m ks seed mode budget]");
    ...
    let ks: Vec<usize> = if ks_arg.contains(',') {
        ks_arg
            .split(',')
            .map(|s| s.trim().parse().expect("bad k list"))
            .collect()
    } else {
