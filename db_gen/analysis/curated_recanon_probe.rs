...
fn canonical_form(circuit: &CircuitSeq) -> Option<Vec<u8>> {
    ...
    let mut canonical = CircuitSeq {
        gates: circuit
            .gates
            .iter()
            .map(|&[target, a, b]| [map[target as usize], map[a as usize], map[b as usize]])
            .collect(),
    };
...
fn run() -> AnyResult<()> {
    ...
    let Some(path) = args.get(1) else {
        eprintln!(
            "usage: curated_recanon_probe COMPOSITE_ROCKS [--stride N] [--max-per-key M] [--top K]"
        );
        std::process::exit(2);
    };
    ...
    (0..256usize)
        ...
        .try_for_each(|shard| -> AnyResult<()> {
            ...
            let mut finish =
                |stat: &mut KeyStat, seen: &mut HashSet<u128>, local_top: &mut Vec<KeyStat>| {
