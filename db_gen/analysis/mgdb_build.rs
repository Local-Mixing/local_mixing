...
fn main() {
    ...
    let dir = a.next().expect(
        "usage: mgdb_build <curated_dir> [--count N] [--min A] [--max B] [--out D] [--seed S]",
    );
    ...
    eprintln!(
        "[mgdb] scan done: {} target permutations, identity pool {} entries",
        targets.len(),
        pool.len()
    );
    for &l in &needed_ls {
        eprintln!(
            "[mgdb] short-identity pairs of length {l}: {}",
            short_pool[&l].len()
        );
    }
    ...
    writeln!(
        manifest,
        "file\tmin_gates\tstore_friends\tkept\tminimal_circuit"
    )
    .unwrap();
    ...
    for (idx, fr) in targets.iter().enumerate() {
        ...
        let reference = fr
            .iter()
            .filter(|f| f.len() == mn)
            .min_by_key(|f| f.to_vec())
            .unwrap()
            .clone();
        let want_total = if per_size > 0 {
            per_size * target_lens.len()
        } else {
            count
        };
        ...
        while kept.len() < want_total && attempts < 40 * want_total as u64 {
            ...
            let (f, ident): (&Vec<G57>, Vec<G57>) = if target_lens.is_empty() {
                ...
                let t = if per_size > 0 {
                    let open: Vec<usize> = target_lens
                        .iter()
                        .copied()
                        .filter(|l| quota[l] > 0)
                        .collect();
            ...
            let map: std::collections::HashMap<u16, u16> = iw.iter().copied().zip(slots).collect();
            let ident_m: Vec<G57> = ident
                .iter()
                .map(|&[t, p, n]| [map[&t], map[&p], map[&n]])
                .collect();
            ...
            *len_hist.entry(cand.len()).or_insert(0) += 1;
            if per_size > 0 {
                *quota
                    .get_mut(&cand.len())
                    .expect("kept length outside target lens") -= 1;
            }
            kept.push(cand);
        }
    ...
    assert_eq!(
        rej_verify, 0,
        "verification rejects should be impossible — investigate before using these DBs"
    );
}
