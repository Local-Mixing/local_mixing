...
fn main() {
    ...
    let dir = a
        .next()
        .expect("usage: sgdb_build <curated_dir> [--count N] [--out F] [--seed S]");
    ...
    'shards: for &s in &shards {
        ...
        for fr in entries {
            ...
            let sevens: Vec<usize> = (0..fr.len()).filter(|&i| fr[i].len() == 7).collect();
    ...
    assert_eq!(
        rej_verify, 0,
        "verification rejects should be impossible — investigate before using this DB"
    );
}
