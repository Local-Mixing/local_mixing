...
fn main() {
    ...
    let inp = a
        .next()
        .expect("usage: sgdb_substitute <in> <sgdb> <out> [--seed S] [--verify K]");
    ...
    let mask = if wires >= 1024 {
        U1024::MAX
    } else {
        (U1024::one() << wires) - U1024::one()
    };
    let mut bad = 0usize;
    for _ in 0..verify_k {
        let mut b = [0u8; 128];
    ...
    assert_eq!(
        bad, 0,
        "FUNCTIONAL VERIFY FAILED: {bad}/{verify_k} states mismatched"
    );
