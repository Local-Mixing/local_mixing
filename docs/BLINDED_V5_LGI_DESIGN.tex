...
fresh-pair top-up runs until $|\rho| \ge \doc{min\_mask}$, a \textbf{hard masking
floor} --- never bare, and never thinly masked even in a rare draw where the open
pairs cancel), realises
...
firing} of \S3 splits the fire into two halves, \textbf{each independently shuffled}
(the monomials all target $c$ and commute), and straddle-opens one of $c$'s \lgi{}
opens between them.

\paragraph{Band re-randomisation} is woven through the same pass as \textbf{bursts}
(\S1d): a plan of \doc{straddle\_slots} STRADDLE slots (auto $\approx m/(4K)$, at the
safe side of a $\approx1024$ thinning knee) and \doc{rerand\_repair} REPAIR slots
(default 0), shuffled together. Each slot targets one band wire $b$ with a burst of
$F \approx 8\mathrm{K}$ updates. A STRADDLE slot first \textbf{closes} the masks
reading $b$ (thins masking past the knee --- hence the slot budget); a REPAIR slot
brackets the burst with each such mask's $b$-reading \g{} (old $b$ cancels, new $b$
re-adds, so the mask stays open --- no thinning). The slot rate is
\textbf{calibrated ahead} --- one slot every $\mathit{total\_steps}/\mathit{slots}$
primitive steps --- so the last slot lands \emph{during} the pass with \textbf{no
end-flush} (emitting rerands after the final $A$-gate would be pointless); the slot
count is thus fixed but the total is variable within bounds. Being in the same pass,
it protects the straddle opens automatically.
