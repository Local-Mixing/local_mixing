...
\noindent\textcolor{red}{\textbf{Partly superseded (2026-07-25) by}
\texttt{docs/BAND\_HARDENING}}\emph{, which (i) applies the fold shuffle of
\S4 to the wide production path---it had reached only the narrow path---(ii)
puts the band inside the input slice, and (iii) builds the rolling band that
\S3 lists as the unfixed weakness.}
...
condition on one. \emph{Built since, by a different route: the band variable's
\textbf{location} rolls (RG2's swap extended across the carrier/band boundary)
instead of the wire being retired---see} \texttt{docs/BAND\_HARDENING} \S4.
...
Fragments are now shuffled within each fold. \emph{(Correction: as shipped in
this pass the shuffle reached only the narrow fold path; the wide production
path still emitted in odometer order until} \texttt{docs/BAND\_HARDENING} \S2
\emph{.)} This is the cheap half of the fix;
