...
Second, \textbf{band 256 is the shape to ship}. At 512 wires it is
\emph{narrower} than the current 558, yet the carrier and band populations are
equal in size with write distributions sitting on top of each other. That is
qualitatively different from rolling a 46-wire minority: there is no small
subpopulation left for a windowed census to narrow down --- the residual that
\code{BAND\_HARDENING} conceded rolling could not fix. The restricted degree-3
attack cost rises from $\binom{46}{3}\approx 1.5\cdot 10^4$ regressors to
$\binom{256}{3}\approx 2.8\cdot 10^6$.
...
both with \code{--prod-band 256 --prod-fill-nl 2 --prod-roll 1} at the
production sandwich. Auto band width is
