...
The obvious reading of the cap is that a fragment with at most two controls is
reachable. That reading is wrong, and the error is algebraic. Over the wires
$\{h, x, y\}$ the $X$-free span of the g57/\textsc{cnot} vocabulary is
\[
  \langle\, x,\; y,\; 1 \oplus xy \,\rangle,
\]
which does \emph{not} contain $xy$. A width-2 conjunction with $\text{comp}=0$
is therefore invisible to a store built from g57 circuits however narrow it
looks, while $\lnot x \cdot \lnot y = (1 \oplus \lnot x \cdot y) \oplus x$ is
already inside. \code{--prod-g57-narrow} re-realises every width-$\leq$2 fold
fragment through the g57 form, at a cost of one or two gates instead of one,
with the residual constant going to the ledger exactly as the narrow path
already does. At $n{=}128$, band 256, plan $[1,2,3,3]$, the store's match rate
on the result rises from $0.3506$ to $0.4151$ for $+4.5\%$ gates. The gain is a
fraction rather than all of it precisely because the $\lnot x \lnot y$ corner
...
\code{--prod-cg-jitter} gives a percentage of values one extra high-degree
term, so the count becomes $(1 + k_a)(1 + k_b)$ and varies with which values
the source gate happens to read. Jitter adds terms and never removes them: the
operating point a build commits to is its weakest value, so removing a term
anywhere would move the commitment, while adding one only raises a value above
the floor.
...
that a value's terms are variable-disjoint. What the fill establishes is
\emph{marginal} balance, one wire at a time, and the accompanying test checks
exactly that marginal. A mask multiplies three band wires together. Two
concrete gaps: the nonlinear fill draws each wire's pivot \emph{with}
replacement across band wires, and each wire's exclusions cover only its
\emph{own} pivot, so one wire's pivot can re-enter another's linear part. A
reserved pivot block (distinct pivots, the whole pivot set excluded from every
wire's non-pivot material) makes the band exactly uniform on $\{0,1\}^b$ for
\emph{any} nonlinear part, at zero gate cost --- see
\code{docs/BAND\_INDEPENDENCE}. Until then, treat \eqref{eq:piling} as a model
the measurements happen to track.
...
\code{--prod-band} & \code{PROD\_BAND} & band width; 0 = auto \\
...
both with \code{--prod-band 256 --prod-fill-nl 2 --prod-roll 1
--prod-epoch 5 --prod-refill-data 50} at the production sandwich, which is
\code{ProdConfig::production\_single} in the tree.

\textbf{Every hardening lever defaults OFF}, which is right for
reproducibility and wrong for anyone generating a circuit: the defaults (auto
band 56, no nonlinear fill, no rolling, no epochs) produce a materially weaker
gadget than any measurement in this document describes. Use the preset. Auto band width is
$\max(\lceil\sqrt{4\,n\,k_{\mathrm{total}}}\,\rceil, 6, \max\deg + 3)$, where $n$
is the \emph{entry point's} value count --- the sandwich width $2n$, not the
source circuit's $n$. \code{--prod-single 0} reproduces the previous build
bit-identically.
