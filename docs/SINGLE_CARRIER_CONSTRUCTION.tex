...
part, at zero gate cost --- see \code{docs/BAND\_INDEPENDENCE}. This is now the
\emph{only} unmet hypothesis of \eqref{eq:piling}, and it is the natural
explanation for the residual excess over theory that survives with disjointness
enforced ($+0.008$ at $A{=}4$, $+0.020$ at $A{=}6$, $n{=}64$). Until it lands,
treat \eqref{eq:piling} as a model the measurements happen to track.
