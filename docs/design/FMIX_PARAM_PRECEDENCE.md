...
> Markdown rendering of `docs/design/FMIX_PARAM_PRECEDENCE.tex`. The PDF is authoritative for figures, diagrams and display math.
...
explicit CLI > preset (`--gss` over `--db-mixing`)
> shipped default
...
| `--db-mixing` | pcv 0.4, 9/9, **pmg 0.6**, dsc on | pcv 0.9, 12/12, **pmg 0**, dsc on |
| `--db-mixing --gss` | pcv 0.5, 6/6, pmg 0.5, dsc off | pcv 0.95, 12/6, pmg 0, dsc on |
...
**Row 9** shows GSS sits above db_mixing where they overlap
(`p_mingen` 0.5 not 0.6), and that db_mixing's non-DB block still applies.
...
| `src/programs/fmix/mod.rs` | `DbLayer`, `DbLayer::shipped/gss/phase_a`, the `cli.over(…)` line, |
|---|---|
|  | the inert-knob check, the effective-per-mode banner |
| `src/engine/mixer/mod.rs` | `MixParams::db_knobs`, `ResolvedDbKnobs`, the `active_*` accessors |
| `docs/history/research/POSTMIX_MANUAL.md` | §2.1.2 (per-geometry, per-mode), §2.1.3 (the GSS profile) |
