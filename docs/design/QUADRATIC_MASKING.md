# Quadratic masking: the LGI-based compute module

*Design rationale, construction, parameters, and measurements for the
locally-geodesic-identity (LGI) computation stage.*

The current API names this family **quadratic masking**; Ran balanced and
blinded V5 are historical names used throughout the measurements below.
`QuadraticMaskingParams::managed_gss(seed, source_wires)` preserves the verified
managed recipe: balanced masks, K=2, max-open=3, min-open=2, quadratic fire,
auto band width, public I/O, and source-prefix band seeding. **Dirty fire helpers
use band wires by default**, while **burst refresh can read both data and band
wires** (`burst_band_only=false`). These are separate controls.
`BV5_BURST_BANDONLY=1` remains an explicit direct-generator research override.
The earlier reorganization plan's claim that the managed runner pinned this
burst override was stale: the pre-refactor runner scrubbed the BV5 environment
and did not set it. The extraction preserves that verified behavior.
...
Source: [`src/stages/preprocessing/quadratic_masking.rs`](../../src/stages/preprocessing/quadratic_masking.rs).
...
(`security_tests/leakage/fire_corr.rs`) measures the phi (Pearson)
...
Running it: `python security_tests/gauntlet/gauntlet.py all --arms
...
the coverage-rule counts. The gauntlet arms (§5.7) run from `security_tests/gauntlet/gauntlet.py`
(`--arms blindedv5,blindedv5_balanced,blindedv5_balanced_mo2 --n-wires 32`); the idle-bare
census of a gadget is `security_tests/leakage/bare_census.py`; the correlation scan is `fire_corr`.
