...
# Stages 3-4 piecewise-parallel mixing (docs/design/FMIX_PIECEWISE.md).
# Choose pieces OR min_block_size; leave the other blank. Both blank = serial.
# min_block_size derives P=max(1,current gates / size) each round (integer
# division); it is NOT a hard minimum for the smaller shifted end blocks.
# Blank threads = P+1 for fixed pieces, available CPUs for automatic sizing.
pieces =
min_block_size =
piece_threads =
...
| `pieces` | `1` (serial when no size option is set) | `1..=64`; mutually exclusive with `min_block_size` |
| `min_block_size` | unset | `2..=1000000000`; derive P = max(1, floor(current gates / size)) each round; mutually exclusive with `pieces` |
| `piece_threads` | `pieces + 1` for fixed counts; available CPUs for automatic sizing | `1..=1024`; only with `pieces > 1` or `min_block_size` set; lifecycle setting |
...
`stop_after`, `force_from`, `piece_threads`, filter selection, and
build-location controls are lifecycle settings and are not locked; `pieces`
and `min_block_size` are locked recipe values. The manifest schema is `gss_command_recipe=3` (the
piecewise feature added the `pieces` line and changed the fmix binary and the
driver script), so run directories created by an earlier version cannot be
resumed through `gss`; finish them with the direct Bash driver. All eventual
stores and future-stage knobs should nevertheless be chosen before the first
partial run, because the whole future recipe is locked immediately. Frozen stores are identified by
...
`docs/history/research/CARRIER_GADGETIZATION_SUMMARY.tex`. In brief, verified live Phase-A MIX
...
**swap-refresh symmetric construction** (`docs/history/research/SWAP_REFRESH_REDESIGN.md`):
...
With `pieces = P > 1` or `min_block_size = B`, the stage runs
**piecewise-parallel rounds**. Automatic sizing computes
`P = max(1, floor(current gates / B))` at the start of every round; B only
sets P and does not limit the smaller shifted end blocks. When P changes,
the cuts rebalance for the new count while retaining the shift phase.
The shared driver (`docs/design/FMIX_PIECEWISE.md`) cuts the circuit each round
into P contiguous
pieces of about equal length, mixes every piece in parallel on one shared store
handle, concatenates them back in order, verifies the whole circuit against the
stage input, and steps the unchanged profile controller once. The next round
cuts at the midpoints between the previous seams (the half-slice shift; a
shifted round has P + 1 pieces, the two end pieces half a slice long), so
material at an old boundary is mixed in the following round. The round length
is the controller cadence, so the size schedule and the lever updates are the
serial ones; the pieces are steered with a fixed lever and a proportional size
setpoint. Output depends only on `(input, flags, seed)`, not on the thread
count. Per-piece report lines are suppressed; the driver prints one
`[fmix] pieces: round ...` line and one `[fmix] mv=` line per round, so the
report-point count in `gss_mix.log` is about the number of rounds. Known
difference from the serial walk: no window or twist crosses a cut within a
round, and the rare phase-A twist windows are capped at the piece length.
...
With `pieces = P > 1` or `min_block_size` set, the split stage uses the same rounds: the
256 canaries are planted once on the whole circuit and travel with their gates,
every piece splits to its own exhaustion or failure limit, and rounds repeat
with shifted cuts until the merged circuit has no g57 left (or six rounds, or a
round without any split — both reported as the stage's end reason). The
`split stage ENDED`, `split spans` and `canary deciles` lines are printed once,
from the whole-circuit counters, in the serial formats (the span histogram stays
frac-of-circuit). The merged `splitB.state` is a normal v2 state, so stage 5 is
unchanged. Behavioural difference to know: a NOT twist's brackets are drawn
inside one piece, so spans are capped at the piece length, and a g57 split
inside a piece cannot later be split across the whole circuit.
...
`docs/design/FCOMPRESS_TRANSPORT_AND_PACKING.md`): one generalized gate per maximal
...
- **State v2**: stages 4–5 write resume states; stage 5's resume of the
  ended split stage never re-arms it (the tri-state contract). A piecewise
  stage 4 writes the same v2 state from the merged circuit (`moves` = the
  summed piece moves, which stage 5's absolute `--moves` builds on).
- **Pieces**: with `pieces > 1` or automatic sizing, stage 3 and stage 4 outputs are still a
  function of `(input, flags, seed)` only — the thread count never changes a
  result. `pieces` and `min_block_size` are locked recipe values; changing either mid-run is a
  recipe change. The crossing walk (stage 5) stays serial.
