...
Code: `src/preprocessing/blinded_v5.rs`, `src/bin/gen_sandwich_gadget.rs`,
`security_tests/preprocessing/quadratic_masking.rs`, `scripts/gss_mix.sh`;
red team: `security_tests/leakage/fire_corr.rs`; harness: `tests/gauntlet/`.
Design doc (rationale in context): `docs/design/QUADRATIC_MASKING.md` §2, §4, §5.6–5.7, §6, §7.
PDF twin of this note: `docs/history/research/BALANCED_MASKS_AND_COVERAGE_20260907.pdf`.
...
`fire_corr` (`security_tests/leakage/fire_corr.rs`) measures the phi (Pearson)
...
census of open masks (`security_tests/leakage/bare_census.py`) found 448 interior bare
...
The rules (each measured before and after; the census script is `security_tests/leakage/bare_census.py`):
...
7. **Minimum of two open masks** (`min_open`, default 2; added after the commit review).
   ...
   `security_tests/leakage/open_mask_profile.py`: 26.5% of covered wire-time at one mask (3.0%
...
| `src/bin/gen_sandwich_gadget.rs`, `blinded_v5_gadgetize.rs` | env knobs `BV5_BALANCED` (both), `BV5_BAL_SEED` and `BV5_BURST_BANDONLY` (sandwich driver; `max_open` was already `BV5_MAX_OPEN` / positional); balanced band seed |
| `scripts/gss_mix.sh` | `--bv5-max-open N`, `--bv5-balanced 0\|1`, `--bv5-min-open N` |
| `security_tests/leakage/fire_corr.rs` (+ `Cargo.toml` bin), `security_tests/leakage/bare_census.py`, `security_tests/leakage/open_mask_profile.py` | new tools |
| `security_tests/gauntlet/gauntlet_gen.rs`, `gauntlet.py`, `gauntlet_audit.rs`, `README.md`, `TESTING_PIPELINE.md` | blinded-V5 arms, `--n-wires`, docs |
