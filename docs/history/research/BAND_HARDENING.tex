...
\usepackage{parskip}
\newcommand{\xor}{\oplus}
\title{\textbf{Band hardening: the band joins the slice, and starts moving}\\[2pt]
\large Three changes to the new gadgetizer}
\author{}
\date{2026-07-25}
\begin{document}
\maketitle

\noindent\emph{Assumes} \texttt{docs/PRODUCT\_SHARE\_ENCODING} \emph{and its
update} \texttt{docs/PRODUCT\_SHARE\_UPDATE}. \emph{For the construction from
scratch, see} \texttt{docs/NONLINEAR\_GADGETIZATION}.

\section{Summary}

Three weaknesses of the product-share gadgetizer, all of them
\textbf{syntactic}---visible by reading the gate list, without executing
...
clock needing no execution.
& Shuffled on the \emph{narrow} path only; the production \emph{wide} path
still emitted in odometer order.
& Shuffled on both paths. \\[4pt]
The band was outside the input slice: setting band wires to nonzero
\emph{provably} changed nothing, so two queries separate ``band'' from ``aux''.
& Slice $=$ aux wires only.
& Slice $=$ every non-data wire, and the guarantee is now \textbf{exact for all
slices} rather than exact-for-some / heuristic-for-the-rest. \\[4pt]
Band wires body-static: never written between the ports, so trivially
identifiable---which is exactly what a restriction adversary needs.
& Write census separates band from carriers perfectly (band 10--23 writes,
carriers 175--928).
& \textbf{Rolling band}: distributions fully overlap, no threshold separates
...
with and without rolls (\S7). Rolling costs $+12.4\%$ gates at one roll per
inter-SG gap. A fourth question---whether the rolling band lets us \emph{shrink}
the band to the minimum correctness needs---is answered in \S6, and the answer
is no: the minimal band is broken by an 800-regressor attack.

\section{Fold-order randomization, actually applied}

The previous pass reported that fold fragments are shuffled within each fold.
That was true of \texttt{fold\_cg\_narrow} only. The wide path---the production
...
fragment of one fold XORs into the \emph{same} value's two carriers, and reads
only \emph{other} values' carriers plus band wires. No fragment reads what
...
\section{The band is part of the slice}

\subsection{The weakness}

The gadget is correct on its low $n$ wires for \emph{arbitrary} junk on every
other wire; the slice-zero preblock is what turns that into ``computes $C$ only
on the all-zero aux slice''. But the preblock only ever read the \emph{aux}
...
The preblock now spans $\mathit{data}\,[0,n) \mid \mathit{aux}\,[n,2n) \mid
\mathit{band}\,[2n,2n{+}b)$ and the slice is \textbf{every non-data wire}. The
gate vocabulary is unchanged (positive-polarity CNOTs $x_t \mathrel{\xor}= s_w$
and CCNOTs $x_t \mathrel{\xor}= x_c \wedge s_w$, so every gate is individually
dead on the all-zero slice), but the construction is new, and it upgrades the
guarantee.

\textbf{Old guarantee.} The CNOT parity matrix over the aux columns was
resampled until invertible, which made the disturbance exact on slices that fire
no CCNOT, and \emph{heuristic} elsewhere---a slice firing CCNOTs was fixed only
if its fired subsequence composed to the identity \emph{and} the conjugated
translations cancelled. Vanishingly unlikely, measured at $0/50\mathrm{k}$ by
$n{=}8$, but not a theorem. With $b$ extra columns it could not even have been
stated: an $n\times(n{+}b)$ parity matrix always has a kernel, so a CNOT-only
block \emph{provably} fixes $2^{b}$ nonzero slices. The degree-2 gates are what
make the guarantee expressible at all.

\textbf{What is built.} The block stays unstructured: the target and the data
...
two-control budget becomes \emph{three}-control gates
$x_t \mathrel{\xor}= x_a \wedge x_b \wedge s_w$, which is what keeps the
...
\textbf{checked and redrawn} rather than argued: exhaustively over every slice
and every input where the space fits ($n+b \le 20$, which is exactly the regime
...
\subsection{An exactly-pinning variant, and why it is not used}

It \emph{is} possible to make the property a theorem, and the construction is
...
gate adds $e_t\,m(x_R)$ for a monomial $m \in \{1, x_c, x_a x_b\}$, and since no
gate can change another's monomial they all commute:
\[
S(x,s) \;=\; x + \sum\nolimits_{(t,m)} q_{(t,m)}(s)\; e_t\; m(x_R),
\]
with every $q$ a \emph{linear} form in $s$ and the monomials independent
...
never disturbed; and the whole disturbance is a function of $x_R$ \emph{alone}
---nine bits of the input at production sizing---so an adversary who sets
$x_R=0$ collapses the block to a translation. \textbf{Two layers repair both}:
...
\]
and $z=x$ forces $F_1 \equiv 0$ from the first line and then $F_2 \equiv 0$ from
...
\textbf{unchanged}. Every data bit now influences the disturbance, and the input
set that kills the nonlinearity shrinks from a $2^{\,n-|R|}$ subcube to a single
point.

\textbf{Why it is not used: the mixing barrier.} Layer-2 gates read exactly what
layer-1 gates write, so the two halves do not commute and \emph{cannot} be
shuffled into each other. That ordering survives the final
\texttt{commuting\_shuffle} and every downstream mixing move that preserves the
...
carrier/band boundary. One \textbf{roll} picks a band variable and a uniformly
...
\begin{itemize}
\item if the partner was another band wire, the two variables trade locations;
\item if it was a carrier of some value, \texttt{GadgetState::pairs} is
re-pointed at the vacated band wire---the carrier and the band variable have
\emph{swapped roles}, and the wire that was band is now a live carrier written
...
carriers and band wires remain a \textbf{partition} of the wire space (so a fold
target is never also a mask literal), and mask slots name band
\textbf{variables}, not wires---physical literals are resolved through a
location map at emission time, so the dedup set, re-sourcing, and the final
strip are all invariant under rolling.

Two consequences elsewhere. The band fills take a wire \emph{list} rather than a
...
\begin{tabular}{lrr}
\toprule
 & \texttt{--prod-roll 0} & \texttt{--prod-roll 1} \\
\midrule
gates & 283{,}561 & 318{,}813 ($+12.4\%$) \\
rolls & 0 & 7{,}919 \\
writes/wire, carriers & 209--1004 & 284--969 \\
writes/wire, band & 10--22 & \textbf{282--867} \\
reads/wire, carriers & 272--804 & 625--3287 \\
reads/wire, band & 9725--14712 & \textbf{702--3421} \\
width-1 share of writes, carriers & 0.033--0.115 & 0.050--0.165 \\
width-1 share of writes, band & 0.556--0.826 & \textbf{0.078--0.155} \\
separable by any of the above? & \textbf{yes} & no \\
unwritten wires, 50k window & \textbf{exactly 56} & 0 \\
unwritten wires, 10k window (median) & 68 & 13 \\
\bottomrule
\end{tabular}
\end{center}

\noindent (Four rolls per gap was also measured: it cuts the 10k-window figure
...
residual \emph{local} statistic (a band variable still sits still for
$\sim$2000 gates at roll 1) and costs four times as much, so \textbf{one roll
per gap is the recommendation}; \texttt{--prod-roll} is off by default so the
legacy build stays bit-identical.

\textbf{Write counts are not the only census.} A first version emitted each
roll as three plain CNOTs, which matched the write \emph{counts} but not their
\emph{shape}: nothing but rolls ever writes a wire while it holds a band
...
\textbf{What this does not fix.} A windowed census still narrows the band down:
...
This is why band \emph{width} (\S6) still matters---the two defenses multiply.

\section{Two follow-ups: quadratic slicing, and the missing instrument}

\subsection{The preblock was affine in $x$}

With one data control per gate, fixing any slice $s$ turns every preblock gate
into a constant flip or a transvection, and those compose to an \emph{affine}
map: $S(\cdot,s)$ has degree 1 in $x$, fully determined by $n{+}1$ queries. The
...
The block now also emits $x_t \mathrel{\xor}= x_a \wedge x_b \wedge s_w$ with
both data controls in $R$. The exactness theorem survives \emph{unchanged in
form}: every gate still writes only $T$ and reads only $R$, so they all still
commute, and the composite is
\[
S(x,s) \;=\; x + \sum\nolimits_{(t,m)} q_{(t,m)}(s)\, e_t\, m(x_R),
\qquad m \in \{1,\; x_c,\; x_a x_b\},
\]
...
rank check simply gains rows keyed by data \emph{pairs} alongside the singles.
It costs nothing (the gate budget is fixed) and a new test confirms a genuine
second-order term $S(a\xor b)\xor S(a)\xor S(b)\xor S(0)\neq 0$ on some slice.

\subsection{A statistical readout}

Every measurement in this project has been made with \texttt{hmap\_affine},
which asks whether $C_i$'s bit is an \textbf{exact} $\mathrm{GF}(2)$ function of
...
$c_0 \xor c_1$ is wrong only when the mask sum fires, so piling-up over
monomials firing with probability $2^{-\deg}$ predicts a best-predictor
agreement of $57\%$, $64\%$, $78\%$, $87.5\%$ for
$[2,2,3,3]$, $[2,3,3]$, $[3,3]$, $[3]$ --- while the exact-span plate reports
$\mathrm{meanH}\approx 0.498$ for all of them.

\texttt{red\_team\_tests/bin/heatmaps/hmap\_stat.rs} measures that directly: per cell and target bit,
...
random target. At $n{=}128$, 31 rows $\times$ 64 columns, 8192 samples:
...
\midrule
plain gadget & 0.525 & \textbf{0.907} & 0.970 & 1.000 \\
$[2,3,3]$ (shipped) & 0.526 & \textbf{0.609} & 0.624 & 0.641 \\
$[3,3]$ & 0.525 & \textbf{0.703} & 0.741 & 0.781 \\
$[3]$ & 0.526 & \textbf{0.803} & 0.845 & 0.875 \\
\bottomrule
\end{tabular}
\end{center}

\noindent (Per-row max over columns, interior only: both axes have unencoded
ends, and cell $(0,0)$ --- C's input state against G's input wires --- reads
$1.0$ in \emph{every} build, encoded or not. Trimming the outer tenth of both
...
separates them and prices the $k{:}3\to2$ cut at $0.609 \to 0.703$
predictability for its 25\% of the gates. And $[3]$, at $0.803$, sits only
$0.10$ below the \emph{unencoded} gadget --- consistent with its exact-ridge
failure ($\rho = 0.995$), and confirming two terms as the floor.

\subsection{One product term closes most of that gap}

\texttt{--and-wires} gives the predictor a single
$(w_p \xor a)(w_q \xor b)$ over a declared wire set on top of its best XOR
predictor: the \emph{degree-2-capable} adversary, and exactly what cancels a
...
\midrule
$[2,3,3]$, roll 0 & 0.602 & \textbf{0.693} & $+0.090$ \\
$[3,3]$, roll 0 & 0.718 & \textbf{0.728} & $+0.010$ \\
\bottomrule
\end{tabular}
\end{center}

The base degree-2 term \emph{is} a product of two band wires, so one AND removes
...
the 25\% saving is correspondingly cheaper than \S5.2 alone suggests.

Rolling changes the picture again. The same search on a \texttt{--prod-roll 1}
...
the syntactic census of \S4; it also raises the price of this statistical
adversary, who must either locate the band per snapshot or widen the AND search
to all $\binom{568}{2}$ pairs and pay the higher noise floor. Every number here
is a lower bound.

\section{How wide should the band be?}
...
assertion is the only hard constraint---$\binom{b}{d}2^{d}$ distinct factor
tuples must cover the $n\cdot k_{\mathrm{total}}$ live slots with headroom---and
...
\textbf{It is also broken.} Band width is not a correctness parameter, it is
the \emph{dimension} of the space the mask terms live in, and therefore the
...
$\binom{b}{3}$ regressors: \textbf{220 at $b{=}12$, 27{,}720 at $b{=}56$}.

Measured, same $C$, same $c$-step, an adversary with an identical and cheap
budget (all triples over 12 wires, $\sim$800 regressors in total):

\begin{center}
\begin{tabular}{lrrrrl}
\toprule
build & depth & depthMed & $\rho$ & meanH & verdict \\
\midrule
$b{=}12$, roll 0 & 0.259 & 0.266 & \textbf{1.000} & 0.478 & ALIVE---broken \\
...
perfect $\rho=1$ progress ridge, for an attack that fits in a few hundred
regressors. Rolling saves it, but only against \emph{this} adversary, which
...
attack expensive \emph{once you know where the band is}, rolling makes knowing
where it is expensive. Dropping to the minimum keeps only the second factor,
which is not airtight, and saves 7.7\% of the wires. \textbf{Keep the band
wide}---if anything the auto size ($\sqrt{4nk_{\mathrm{total}}}$, chosen for
slot space, not for this) is worth revisiting upward, since $\binom{b}{3}$
...
adversary, matched $c$-step 30 / 101 rows and $\sim$152 columns:

\begin{center}
\begin{tabular}{lrrrrr}
\toprule
build & gates & depth & depthMed & $\rho$ & meanH \\
\midrule
plain gadget (control) & 85{,}354 & 0.356 & 0.363 & \textbf{1.000} & 0.475 \ (ALIVE) \\
$[2,3,3]$+nlfill, roll 0 & 281{,}436 & 0.043 & \textbf{0.000} & 0.018 & 0.498 \ (dead) \\
$[2,3,3]$+nlfill, roll 1 & 316{,}926 & 0.043 & \textbf{0.000} & 0.018 & 0.499 \ (dead) \\
\bottomrule
\end{tabular}
\end{center}
...
exactly such a permutation. The rolls therefore \emph{cannot} move this number,
and the fact that they do not is a consistency check, not evidence of benefit.
The benefit is the syntactic one measured in \S4.

Library suite \textbf{177/177}, five new tests: the wide fold no longer emits
...
\begin{itemize}
\item The statistical readout now exists (\S5.2) but only measures a
\textbf{two-wire} predictor family. A stronger adversary---greedy or LMS fits
over more terms, or products---would give a tighter lower bound; the current
numbers are floors, not estimates.
\item With the instrument in hand, re-price the whole mask plan. $[2,3,3]$
...
\item Deeper fold randomization: cross-fold interleaving, random ESOP re-covers,
$k$-jitter.
\item The CG fold is 76\% of the gadget and costs
$(2+k_{\mathrm{total}}+\epsilon)^{\mathrm{arity}}$ per source gate. The largest
...
\item Roll-rate tuning against a \emph{windowed} adversary rather than the
...
\item Static/syntactic distinguishers are still only instrumented ad hoc: the
census in \S4 was written for this pass, not by a standing tool. Every other
