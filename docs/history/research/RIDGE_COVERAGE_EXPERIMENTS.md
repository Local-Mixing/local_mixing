...
`docs/history/research/RIDGE_COVERAGE_EXPERIMENTS.pdf`. Tools committed this session:
`fmix --gen-snap-every` (`49cee4c8`) and `benchmarks/mixing/pairfloat_exp.rs`
...
heatmap with the **ridge measure** (`security_tests/reporting/plot_hmap_ridge.py`): `depth`
...
python3 security_tests/reporting/plot_hmap_ridge.py M ...
```
