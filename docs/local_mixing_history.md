...
# Local Mixing of Reversible Circuits: History

This is the research history of the project: its constructions, experiments,
attacks, and the reasons the method changed. The narrative and measurements
below retain their original experimental context. References to the "current"
or "complete" method within that narrative describe that point in the history;
they are not a claim that every later implementation has been tested with those
parameters. The separate [current-method documentation](local_mixing_documentation.md)
describes the implementation available today without the experimental journey.

The historical narrative below predates the optional carrier-shuffling update
of September 16, 2026. Its embedded-masking section describes fixed physical
carriers. The [dated implementation update](#embedded-masking-shuffling-update)
records the addition while preserving the earlier discussion and results.
...
<a id="embedded-masking-shuffling-update"></a>

## September 16, 2026: Optional carrier shuffling

Embedded masking now has an optional preprocessing transform which moves a
logical target between physical carrier wires during its fire block. This is
different from the fire-unit order shuffles already described above. It remains
the same `embedded-masking` mode: `preprocessing.shuffling_segments = 0` leaves
the transform off, and values of at least eight enable it.

The emitter records exact boundaries between complete fire units and mask
atoms, together with the target's live mask support. At selected boundaries,
two CNOTs transfer a target value $X$ and add a temporary mask from a band
value $w$: $(p,q)=(X,w)$ becomes $(w,X\oplus w)$. A role-to-physical-wire map
redirects subsequent gates, and the temporary masks are removed at the end
of the block. The full sandwich returns the data role to its original wire
after the block's final original write, preserving the public input and output
positions. An experimental gauntlet arm may instead carry the layout and decode
through the final map.

The implementation excludes all control dependencies of every remaining atomic
unit that writes the target, including controls on dirty-helper writes. It does
not split those units. Mask-coverage diagnostics are computed before physical
remapping, where the original data and band role coordinates remain valid.
Gauntlet comparisons can now use paired lists of mixer seeds instead of one
hard-coded mixer seed.

The earlier research harness and the in-tree transform use different random
streams and boundary policies, and return-home is an additional construction
choice. Consequently, the historical heatmaps, SAT timings, compression ratios,
and gauntlet counts above are not measurements of this new transform. See
[preprocessing shuffling](local_mixing_documentation.md#preprocessing-shuffling)
for its equations and invariants, and the
[attack methodology](local_mixing_documentation.md#attacks)
for the distinctions a new comparison must preserve.
