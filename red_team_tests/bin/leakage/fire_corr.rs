...
fn main() -> std::io::Result<()> {
    ...
    println!(
        ...
        c.len(), n, g.len(), gw, w * 64, seed
    );
    // random legal inputs: x on wires 0..n
    let x: Vec<Vec<u64>> = (0..n).map(|_| (0..w).map(|_| fastrand::u64(..)).collect()).collect();
    // C: firing vectors
    let mut cst: Vec<Vec<u64>> = x.clone();
    let cn = c.iter().map(|gt| gt.target as usize).chain(c.iter().flat_map(|gt| gt.ctrls.iter().map(|l| l.0 as usize))).max().unwrap_or(0) + 1;
    ...
    let fires: Vec<Vec<u64>> = c
        .iter()
        .map(|gt| {
            let a = gt.ctrls.first().map(|l| cst[l.0 as usize].clone()).unwrap_or_else(|| vec![0; w]);
            let b = gt.ctrls.get(1).map(|l| cst[l.0 as usize].clone()).unwrap_or_else(|| vec![0; w]);
        ...
        .collect();
    if args.c_segments {
        println!("[fire_corr] mode: C STATE BITS (segment after each C gate) vs G segments/increments");
    }
    let f_ones: Vec<u64> = fires.iter().map(|f| f.iter().map(|v| v.count_ones() as u64).sum()).collect();
    ...
    let s_ones: Vec<u64> = segs.iter().map(|s| s.iter().map(|v| v.count_ones() as u64).sum()).collect();
    let i_ones: Vec<u64> = incs.iter().map(|s| s.iter().map(|v| v.count_ones() as u64).sum()).collect();
    let const_segs = s_ones.iter().filter(|&&o| o == 0 || o == w as u64 * 64).count();
    let const_incs = i_ones.iter().filter(|&&o| o == 0 || o == w as u64 * 64).count();
    println!(
        "[fire_corr] evaluated in {:.1}s; constant segments {} ({:.1}%), constant increments {} ({:.1}%)",
        t0.elapsed().as_secs_f64(), const_segs, 100.0 * const_segs as f64 / g.len() as f64,
        const_incs, 100.0 * const_incs as f64 / g.len() as f64
    );
    ...
    let scan = |fv: &Vec<Vec<u64>>| -> Vec<(Best, Best)> {
        fv.par_iter()
            ...
            .map(|(i, f)| {
                ...
                let mut bs = Best { phi: 0.0, k: usize::MAX };
                let mut bi = Best { phi: 0.0, k: usize::MAX };
                for k in 0..segs.len() {
    ...
    let summarize = |res: &Vec<(Best, Best)>, label: &str| -> (f64, f64, usize, usize, usize, usize) {
        ...
        println!(
            ...
            q(&ms, 0.5), q(&ms, 0.9), q(&ms, 1.0), cnt(&ms, 0.3), cnt(&ms, 0.5), args.match_thresh, cnt(&ms, args.match_thresh), ms.len()
        );
        println!(
            "[fire_corr] {label}: max|phi| over INCREMENTS per C gate: median {:.3} p90 {:.3} max {:.3}; >=0.3: {} >=0.5: {} >={:.2}: {} of {}",
            q(&mi, 0.5), q(&mi, 0.9), q(&mi, 1.0), cnt(&mi, 0.3), cnt(&mi, 0.5), args.match_thresh, cnt(&mi, args.match_thresh), mi.len()
        );
        (q(&ms, 0.5), q(&ms, 1.0), cnt(&ms, 0.5), cnt(&ms, args.match_thresh), cnt(&mi, 0.5), cnt(&mi, args.match_thresh))
    };
    ...
    let m = c.len();
    let mg = g.len();
    let mut csv = std::io::BufWriter::new(std::fs::File::create(format!("{}.gates.csv", args.out))?);
    writeln!(csv, "c_gate,c_frac,target,ctrl_x,ctrl_y,fire_rate,max_phi_seg,g_gate_seg,wire_seg,g_frac_seg,max_phi_inc,g_gate_inc,wire_inc,g_frac_inc,null_max_phi_seg,null_max_phi_inc")?;
    ...
    for (i, (bs, bi)) in real.iter().enumerate() {
        ...
        let (gs, gi) = (bs.k, bi.k);
        let gfs = if gs == usize::MAX { -1.0 } else { gs as f64 / mg as f64 };
        let gfi = if gi == usize::MAX { -1.0 } else { gi as f64 / mg as f64 };
        let cx = c[i].ctrls.first().map_or(-1, |l| l.0 as i64);
        let cy = c[i].ctrls.get(1).map_or(-1, |l| l.0 as i64);
        writeln!(
            csv,
            "{},{:.4},{},{},{},{:.4},{:.4},{},{},{:.4},{:.4},{},{},{:.4},{:.4},{:.4}",
            i, cf, c[i].target, cx, cy, f_ones[i] as f64 / nn, bs.phi, gs as i64, if gs == usize::MAX { -1 } else { g[gs].target as i64 }, gfs,
            bi.phi, gi as i64, if gi == usize::MAX { -1 } else { g[gi].target as i64 }, gfi,
            null[i].0.phi, null[i].1.phi
        )?;
        if gs != usize::MAX {
            let (a, b) = (((cf * 10.0) as usize).min(9), ((gfs * 10.0) as usize).min(9));
            grid[a][b] = grid[a][b].max(bs.phi.abs());
        }
        if gi != usize::MAX {
            let (a, b) = (((cf * 10.0) as usize).min(9), ((gfi * 10.0) as usize).min(9));
            grid_inc[a][b] = grid_inc[a][b].max(bi.phi.abs());
        }
    }
    let mut gf = std::fs::File::create(format!("{}.grid.csv", args.out))?;
    writeln!(gf, "# rows: C-gate decile, cols: G-position decile of the arg-max segment; value: max |phi| (segments), then (increments)")?;
    for row in &grid {
        writeln!(gf, "{}", row.iter().map(|v| format!("{v:.3}")).collect::<Vec<_>>().join(","))?;
    }
    writeln!(gf, "#inc")?;
    for row in &grid_inc {
        writeln!(gf, "{}", row.iter().map(|v| format!("{v:.3}")).collect::<Vec<_>>().join(","))?;
    ...
    order.sort_by(|&a, &b| real[b].0.phi.abs().partial_cmp(&real[a].0.phi.abs()).unwrap());
    println!("[fire_corr] top {} (C gate -> segment): c_gate c_frac | phi | g_gate g_frac wire | fire_rate", args.top);
    for &i in order.iter().take(args.top) {
        let b = &real[i].0;
        if b.k == usize::MAX { continue; }
        println!(
            "   c{:<5} {:.3} | {:+.3} | g{:<8} {:.3} w{:<4} | {:.2}",
            i, i as f64 / m as f64, b.phi, b.k, b.k as f64 / mg as f64, g[b.k].target, f_ones[i] as f64 / nn
        );
    }
    let mut oi: Vec<usize> = (0..m).collect();
    oi.sort_by(|&a, &b| real[b].1.phi.abs().partial_cmp(&real[a].1.phi.abs()).unwrap());
    println!("[fire_corr] top {} (C gate -> increment): c_gate c_frac | phi | g_gate g_frac wire", args.top);
    for &i in oi.iter().take(args.top) {
        let b = &real[i].1;
        if b.k == usize::MAX { continue; }
        println!("   c{:<5} {:.3} | {:+.3} | g{:<8} {:.3} w{:<4}", i, i as f64 / m as f64, b.phi, b.k, b.k as f64 / mg as f64, g[b.k].target);
    }
    if !args.probe.is_empty() {
        let ones = |v: &[u64]| v.iter().map(|x| x.count_ones() as u64).sum::<u64>() as f64;
        for tok in args.probe.split(',') {
            let Ok(i) = tok.trim().parse::<usize>() else { continue };
            if i >= m { continue; }
            let k = real[i].0.k;
            ...
            let vals: Vec<String> = tuples[i].iter().zip(names.iter()).map(|(v, nm)| {
                let n1 = ones(v);
                format!("{nm}:{:+.3}", phi(nn, n1, n2, popcnt_and(v, seg) as f64))
            }).collect();
            println!(
                "[probe] C g{i} (target {} ctrls {:?}) best segment G g{k} on w{} (frac {:.3}): {}",
                c[i].target, c[i].ctrls.iter().map(|l| l.0).collect::<Vec<_>>(), g[k].target, k as f64 / mg as f64, vals.join(" ")
            );
        }
    }
    ...
    writeln!(
        ...
        args.c, args.g, m, mg, w * 64, seed, r.0, r.1, r.2, r.3, r.4, r.5, z.0, z.1, z.2, z.3, z.4, z.5
    )?;
    println!("[fire_corr] wrote {}.gates.csv / .grid.csv / .json ({:.1}s total)", args.out, t0.elapsed().as_secs_f64());
    Ok(())
}
