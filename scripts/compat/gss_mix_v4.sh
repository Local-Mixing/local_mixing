...
usage: gss_mix.sh -n N -o RUNDIR [options]
  ...
  --gadgetization-mode MODE
                  ...
                  nonlinear291, or ran-balanced (alias blinded-v5) [ran-balanced;
                  legacy alias: 2223]
...
case "$GADGETIZATION_MODE" in
  product-2223) ;;
  2223) GADGETIZATION_MODE=ran-balanced ;;
  nonlinear193|nonlinear291) ;;
  ran-balanced|blinded-v5|blinded_v5) GADGETIZATION_MODE=ran-balanced ;;
  *)
    echo "FATAL: --gadgetization-mode must be product-2223, nonlinear193, nonlinear291, or ran-balanced (2223 is a compatibility alias)" >&2
    exit 2
    ;;
esac
...
if [ "$FORCE_FROM" -le 2 ] || [ ! -s "$GADGET" ]; then
  ...
  elif [ "$GADGETIZATION_MODE" = ran-balanced ]; then
