...
usage: gss_mix.sh -n N -o RUNDIR [options]
  ...
  --bv5-k K      Ran balanced only: band wires per LGI (LGI length lever).
  ...
  --bv5-min-open N
                 ...
                 instant, exported as BV5_MIN_OPEN [2; range 1..63; must be < max_open]
  ...
  --stop-after K stop after stage K (2, 3, 4, 5 or 6)      [6]
  --force-from K rebuild from stage K even if artifacts exist [unset]
  ...
  --piece-threads T  threads for the piece pool (1..1024; requires pieces > 1 or auto)
...
while [ $# -gt 0 ]; do
  case "$1" in
    -n|-o|-s|--mcd|--gadgetization-mode|--bv5-k|--bv5-max-open|--bv5-balanced|--bv5-min-open|--expand|--hold|--xr|--xb|--xc|--xtdiv|--xmoves|--stop-after|--force-from|--pieces|--min-block-size|--piece-threads)
      if [ "$#" -lt 2 ] || [ -z "$2" ] || [[ $2 == -* ]]; then
...
case "$GADGETIZATION_MODE" in
  ...
  *)
    echo "FATAL: --gadgetization-mode must be ran-balanced, nonlinear193, or nonlinear291" >&2
    exit 2
    ;;
esac
...
N=$(bounded_uint -n "$N" 3 4095)
PIECES=$(bounded_uint --pieces "$PIECES" 1 64)
if [ -n "$MIN_BLOCK_SIZE" ]; then
  [ "$PIECES_SET" -eq 0 ] || {
    echo "FATAL: --min-block-size is mutually exclusive with --pieces (including 1)" >&2
    exit 2
  }
  MIN_BLOCK_SIZE=$(bounded_uint --min-block-size "$MIN_BLOCK_SIZE" 2 1000000000)
fi
if [ -n "$PIECE_THREADS" ]; then
  PIECE_THREADS=$(bounded_uint --piece-threads "$PIECE_THREADS" 1 1024)
...
if [ -n "$BV5_K_ARG$BV5_MAX_OPEN_ARG$BV5_BALANCED_ARG$BV5_MIN_OPEN_ARG" ] && [ "$GADGETIZATION_MODE" != ran-balanced ]; then
  echo "FATAL: --bv5-* flags are only valid with --gadgetization-mode ran-balanced" >&2
  exit 2
fi
if [ "$GADGETIZATION_MODE" = ran-balanced ]; then
  BV5_K=$(bounded_uint --bv5-k "${BV5_K_ARG:-${BV5_K:-2}}" 2 64)
  BV5_MAX_OPEN=$(bounded_uint --bv5-max-open "${BV5_MAX_OPEN_ARG:-${BV5_MAX_OPEN:-3}}" 2 64)
  BV5_MIN_OPEN=$(bounded_uint --bv5-min-open "${BV5_MIN_OPEN_ARG:-${BV5_MIN_OPEN:-2}}" 1 63)
  BV5_BALANCED=$(bounded_uint --bv5-balanced "${BV5_BALANCED_ARG:-${BV5_BALANCED:-1}}" 0 1)
  [ "$BV5_MIN_OPEN" -lt "$BV5_MAX_OPEN" ] || {
    echo "FATAL: --bv5-min-open must be less than --bv5-max-open" >&2
    exit 2
  }
  [ "$((2 * N))" -gt "$((BV5_K - BV5_K % 2 + BV5_BALANCED))" ] || {
    echo "FATAL: Ran balanced requires 2 * N > K rounded down to an even number + balanced; reduce --bv5-k or increase -n" >&2
    exit 2
  }
