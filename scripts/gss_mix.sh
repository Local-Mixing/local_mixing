...
#        convention; Quadratic masking with K=2, max_open=3, min_open=2
#        is the default)
...
usage: gss_mix.sh --source-wires N --run-directory RUNDIR [options]
  ...
  --preprocessing-mode MODE
                  stage-2 representation: quadratic-masking (default; aliases
                  ran-balanced, blinded-v5, blinded_v5), or nonlinear291
  --preprocessing-mask-pair-wires K
                 Quadratic masking only: wires in each mask’s paired component.
                 Exported as BV5_K to gen_sandwich_gadget [2; range 2..64]
  --preprocessing-max-open-masks N
                 Quadratic masking only: open-mask cap per wire, exported as
  ...
  --preprocessing-balanced-masks 0|1
                 Quadratic masking only: balanced masks (CNOT from a fresh band wire
                 per LGI), exported as BV5_BALANCED [1; 0 = plain g57 masks]
  --preprocessing-min-open-masks N
                 Quadratic masking only: minimum open masks per data wire at every
...
GADGETIZATION_MODE=quadratic-masking
...
while [ $# -gt 0 ]; do
  ...
  case "$1" in
    ...
    --gadgetization-mode|--gadget-mode) set -- --preprocessing-mode "${@:2}" ;;
    --bv5-k|--gadget-mask-pair-wires) set -- --preprocessing-mask-pair-wires "${@:2}" ;;
    --bv5-max-open|--gadget-max-open-masks) set -- --preprocessing-max-open-masks "${@:2}" ;;
    --bv5-min-open|--gadget-min-open-masks) set -- --preprocessing-min-open-masks "${@:2}" ;;
    --bv5-balanced|--gadget-balanced-masks) set -- --preprocessing-balanced-masks "${@:2}" ;;
  ...
  case "$1" in
    --preprocessing-*)
      if [ "${PREPROCESSING_FLAGS_SEEN[$1]+present}" ]; then
        echo "FATAL: duplicate preprocessing setting $1; canonical and compatibility aliases cannot both be specified" >&2
        exit 2
      fi
  ...
  case "$1" in
    ...
    --preprocessing-mode) GADGETIZATION_MODE=$2; shift 2 ;;
    --preprocessing-mask-pair-wires) BV5_K_ARG=$2; shift 2 ;;
    --preprocessing-max-open-masks) BV5_MAX_OPEN_ARG=$2; shift 2 ;;
    --preprocessing-balanced-masks) BV5_BALANCED_ARG=$2; shift 2 ;;
    --preprocessing-min-open-masks) BV5_MIN_OPEN_ARG=$2; shift 2 ;;
...
case "$GADGETIZATION_MODE" in
  product-2223|2223|nonlinear193)
    echo "FATAL: $GADGETIZATION_MODE is retired from new GSS runs; resume saved runs through gss with their original configuration" >&2
    exit 2
    ;;
  nonlinear291) ;;
  quadratic-masking|ran-balanced|blinded-v5|blinded_v5) GADGETIZATION_MODE=quadratic-masking ;;
  *)
    echo "FATAL: --preprocessing-mode must be quadratic-masking or nonlinear291" >&2
    exit 2
    ;;
esac
...
if [ -n "$BV5_K_ARG$BV5_MAX_OPEN_ARG$BV5_BALANCED_ARG$BV5_MIN_OPEN_ARG" ] && [ "$GADGETIZATION_MODE" != quadratic-masking ]; then
  echo "FATAL: preprocessing mask flags are only valid with --preprocessing-mode quadratic-masking" >&2
  exit 2
fi
if [ "$GADGETIZATION_MODE" = quadratic-masking ]; then
  BV5_K=$(bounded_uint --preprocessing-mask-pair-wires "${BV5_K_ARG:-${BV5_K:-2}}" 2 64)
  BV5_MAX_OPEN=$(bounded_uint --preprocessing-max-open-masks "${BV5_MAX_OPEN_ARG:-${BV5_MAX_OPEN:-3}}" 2 64)
  BV5_MIN_OPEN=$(bounded_uint --preprocessing-min-open-masks "${BV5_MIN_OPEN_ARG:-${BV5_MIN_OPEN:-2}}" 1 63)
  BV5_BALANCED=$(bounded_uint --preprocessing-balanced-masks "${BV5_BALANCED_ARG:-${BV5_BALANCED:-1}}" 0 1)
  ...
  [ "$((2 * N))" -gt "$((BV5_K - BV5_K % 2 + BV5_BALANCED))" ] || {
    echo "FATAL: Quadratic masking requires 2 * N > K rounded down to an even number + balanced; reduce --preprocessing-mask-pair-wires or increase --source-wires" >&2
    exit 2
  }
...
# Quadratic masking knobs are part of the stage-2 identity (a different K/max_open/balanced
...
note "run=$RUN n=$N preprocessing_mode=$GADGETIZATION_MODE |C|=|D|=$M_CD s=$S_SL slice_gates=$SLICE_G expand=$EXPAND hold=${HOLD}effs x=(r=$XR b=$XB c=$XC tdiv=$XTDIV) $PIECE_DESCRIPTION${PIECE_THREADS:+ piece_threads=$PIECE_THREADS}"
...
if [ "$FORCE_FROM" -le 2 ] || [ ! -s "$GADGET" ]; then
  if [ "$GADGETIZATION_MODE" = quadratic-masking ]; then
    ...
    note "stage 1+2: gen_sandwich_gadget (mode=$GADGETIZATION_MODE; zero-slice preprocessing)"
  fi
