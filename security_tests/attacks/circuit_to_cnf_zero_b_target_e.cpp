...
int main(int argc, char **argv) {
  if (argc != 4) {
    std::cerr << "usage: circuit_to_cnf_zero_b_target_e F.txt out.cnf target128\n";
    return 2;
  }
  ...
  // Input B, wires 128..255, is fixed to 0^128. A and C/Z remain free.
  for (int i = 128; i < 256; i++) clauses.push_back({-state[i]});
