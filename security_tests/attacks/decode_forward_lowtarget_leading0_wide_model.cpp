...
int main(int argc, char **argv) {
  if (argc != 7) {
    std::cerr << "usage: decode_forward_lowtarget_leading0_wide_model F.txt solver.out n target_hex leading_zero_bits target_bits\n";
    return 2;
  }
  ...
  while (std::getline(in, line)) {
    ...
    while (ss >> lit) {
      ...
      if (v >= 1 && v <= n) xbits[static_cast<size_t>(v - 1)] = lit > 0;
    }
  }
  if (!sat) {
  ...
  std::cout << "output " << hex_bits(check, 0, n) << "\n";
  if (n > low_bits) std::cout << "U " << hex_bits(check, low_bits, n - low_bits) << "\n";
