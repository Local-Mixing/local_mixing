...
static std::vector<int> reverse_eval(std::vector<int> bits,
...
static std::vector<int> forward_eval(std::vector<int> bits,
                                     const std::vector<Gate> &gates) {
...
int main(int argc, char **argv) {
  if (argc != 8) {
    std::cerr << "usage: decode_rev_low64_orig_lz_gadget_model "
  ...
  if (total_wires <= 0 || original_wires <= 0 || original_wires > total_wires ||
      leading_zero_bits > original_wires || low_bits < 0 || low_bits > original_wires) {
  ...
  std::vector<int> output_choice(total_wires, 0);
  std::string line;
  bool sat = false;
  while (std::getline(in, line)) {
    ...
    while (ss >> lit) {
      ...
      if (v >= 1 && v <= total_wires) {
        output_choice[static_cast<size_t>(v - 1)] = lit > 0;
      }
    }
  }
  if (!sat) {
  ...
  auto input_bits = reverse_eval(output_choice, gates);
  auto check = forward_eval(input_bits, gates);
  ...
  std::cout << "chosen_output_full " << hex_bits(output_choice, 0, total_wires)
            << "\n";
  std::cout << "output_low_original " << hex_bits(check, 0, original_wires)
            << "\n";
  if (original_wires > low_bits) {
    std::cout << "z_high_part "
              << hex_bits(check, low_bits, original_wires - low_bits) << "\n";
  }
