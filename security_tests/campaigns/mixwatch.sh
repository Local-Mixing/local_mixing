...
#
#   security_tests/campaigns/mixwatch.sh          one snapshot of every run with a log touched in the last 48h
#   security_tests/campaigns/mixwatch.sh -w       refresh every 60s (Ctrl-C to stop)
#   security_tests/campaigns/mixwatch.sh -f NAME  stream a run's log live (tail -f), e.g. -f gad_n128m900_mg80
#
