...
def generate_and_distribute_circuit() -> None:
    ...
    if not setup.get("circuit_generated"):
        cmd = (
            ...
            f"bash security_tests/campaigns/legacy_runner.sh genran -n {INPUT_WIRES} -m {GATES} -d {REMOTE_CIRCUIT}"
        )
...
import sys
import numpy as np
from security_tests.heatmaps import heatmap as plotter
...
"""
    return f"""
cd ~/local_mixing
while pgrep -f "[t]arget/release/(local_mixing_bin|legacy_mixing)" >/dev/null; do sleep 60; done
mkdir -p circuits sgc/parameter_test gadgetized
