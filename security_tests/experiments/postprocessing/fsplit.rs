// Standalone experimental float-and-split post-mixing driver. Takes a mixed
// g57 circuit (or an mpmct1
...
use local_mixing::experimental::split_engine::{Engine, Params, StopReason};
use rand::Rng;
...
fn main() {
    ...
    let gates: Vec<XGate> = if let Some(spec) = &args.gen_random {
        ...
        let input = args
            .input
            .as_deref()
            .expect("--input or --gen-random required");
    ...
    if let Ok(flag) = std::env::var("FSPLIT_DUMP_FLAG") {
        if !flag.is_empty() {
            let out = std::env::var("FSPLIT_DUMP_OUT")
                .ok()
                .filter(|s| !s.is_empty())
                .unwrap_or_else(|| {
                    args.output
                        ...
                        .unwrap_or_else(|| "fsplit_snapshot.txt".to_string())
                });
            let every: usize = std::env::var("FSPLIT_DUMP_EVERY")
                .ok()
                .and_then(|s| s.parse().ok())
                .unwrap_or(200);
            println!(
                "[fsplit] dump signal armed: touch {flag} -> snapshot to {out} (checked every {every} episodes)"
            );
