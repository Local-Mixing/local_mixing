//! Experimental A/B generator: NEW (sliced sandwich -> new-gadgetize) vs OLD (compose_A
...
use local_mixing::preprocessing::gadgets::{
    MaskConfig, ProdConfig, SandwichVariant, compose_a, gadgetize,
    gadgetize_xgates_with_slice_zero_ccnot, sandwich_default_s, sliced_sandwich_with_d,
};
...
fn main() {
    ...
    let sandwich = sliced_sandwich_with_d(&c, &d_xgates, n, s, SandwichVariant::Classic, &mut rng);
    println!(
    ...
    let old_xgates: Vec<XGate> = old_gadget
        .gates
        .iter()
