...
`templates/` retains the historical nonlinear193 topologies;
`src/stages/preprocessing/templates/` owns the four runtime nonlinear291
templates. Both contain deterministic `mpmct1` topologies for the `r57`, `nab`, `and`,
...
The Rust GSS adapter in `src/stages/preprocessing/nonlinear291.rs` consumes these files at
