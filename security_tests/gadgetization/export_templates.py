...
Run from any working directory with::
    ...
    python -m security_tests.gadgetization.export_templates
...
# Permit ``python security_tests/gadgetization/export_templates.py`` as well as ``python -m``.
if __package__ in (None, ""):
    sys.path.insert(0, str(Path(__file__).resolve().parents[2]))

from security_tests.gadgetization import nonlinear193, nonlinear291  # noqa: E402
...
def template_destination(filename: str) -> Path:
    directory = RUNTIME_TEMPLATE_DIR if filename.startswith("nonlinear291_") else TEMPLATE_DIR
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="optional single export directory (default: each variant’s owning directory)",
    )
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    destinations = (
        {args.output_dir: rendered}
        if args.output_dir is not None
        else {
            directory: {name: text for name, text in rendered.items()
                        if template_destination(name).parent == directory}
            for directory in TEMPLATE_DIRECTORIES
        }
    )
    if args.check:
        problems = [problem for directory, bodies in destinations.items()
                    for problem in _check_templates(directory, bodies)]
        if problems:
            for problem in problems:
                print(problem, file=sys.stderr)
            return 1
        print(f"verified {len(rendered)} templates in their owning directories")
        return 0

    for directory, bodies in destinations.items():
        directory.mkdir(parents=True, exist_ok=True)
        for filename, body in bodies.items():
            (directory / filename).write_text(body, encoding="ascii", newline="\n")
    print(f"wrote {len(rendered)} templates to their owning directories")
    return 0
