...
heatmaps.  Run ``python security_tests/gauntlet/gauntlet.py --help`` from any directory
    ...
    *,
    ...
    if policy["kind"] == "file":
        ...
        config["nonlinear193_source"] = source_signature(
            REPO_ROOT / "security_tests" / "gadgetization" / "nonlinear193.py"
        )
        config["nonlinear291_source"] = source_signature(
            REPO_ROOT / "security_tests" / "gadgetization" / "nonlinear291.py"
        )
    return config
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    # parameterised by n_wires), and gauntlet_gen re-checks the builder's
    # buildmeta against analytic_file_layout, so a width mismatch fails loudly
    # rather than silently. Cost is the real limit: a file arm needs
    # 10*n + 12*k + 2 wires, so xrows (O(gates * wires^2)) grows fast in n.
    ...
    if args.cmd in ("all", "gen") and not gen_binary.is_file():
        parser.error(
            f"missing {gen_binary}; run `cargo build --release --features security-tools --bin gauntlet_gen --bin gauntlet_audit`"
        )
    if args.cmd in ("all", "audit") and not audit_binary.is_file():
        parser.error(
            f"missing {audit_binary}; run `cargo build --release --features security-tools --bin gauntlet_gen --bin gauntlet_audit`"
        )
