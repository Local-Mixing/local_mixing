...
fn parse_dist(s: &str) -> Dist {
    if s == "uniform" {
        ...
        Dist::Fix {
            lo: parts[0].parse().expect("fix LO"),
            hi: parts[1].parse().expect("fix HI"),
        }
    } else {
...
fn main() {
    ...
    let d_start = args
        .d_from_end
        .map(|n| d.len().saturating_sub(n))
        .unwrap_or(args.d_start)
        .min(d_end);
    ...
    println!(
        "[hmap] distribution: mean={:.3} std={:.3} bits (of {} wires)",
        mu, sigma, nw
    );
    ...
    println!(
        "[hmap] wrote {} ({}x{} f32) and {}.meta.json",
