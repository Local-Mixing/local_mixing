#!/usr/bin/env python3
"""Render a heatmap plate at TRUE 1:1 — one image pixel per matrix entry.

Values and units are preserved: hmap measures differing-bit counts,
hmap_affine measures reconstruction error, and hmap_stat measures agreement.
Choose a color range appropriate to the generator's metric.
...
def main():
    ...
    print(f"  H range in crop: {S.min():.4f} .. {S.max():.4f}   "
          f"colour scale {vmin:.4f} .. {vmax:.4f}")
    if a.annotate:
        cmn = S.mean(axis=0)
