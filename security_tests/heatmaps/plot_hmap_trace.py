...
``j_idx``.  Plates without mode metadata default to ``window`` mode.  Trace
producers can set ``"mode": "cumulative"`` (``trace_mode`` is accepted as an alias).
...
def render(
    ...
    for panel, (axis, plate, result, title, values, selected) in enumerate(
        ...
        if plate.mode == MODE_WINDOW:
            ...
            if x2.size:
                axis.scatter(x2, y2, s=25, color="white", marker="x", linewidths=0.9, label="2-bit frontier")
