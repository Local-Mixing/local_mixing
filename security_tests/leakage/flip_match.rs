...
fn main() {
    ...
    eprintln!(
        "source {} gates/{} wires ; pred {} gates/{} wires ; {} samples",
        sg.len(),
        s_wires,
        pg.len(),
        p_wires,
        ...
        for i in 0..segs.len() {
            for j in (i + 1)..(i + 1 + kwin).min(segs.len()) {
                ...
                index
                    .entry(hash64(&d))
                    .or_default()
                    .push((w as u32, i as u32, j as u32));
                npairs += 1;
            }
        }
    }
    ...
    for gi in 0..sg.len() {
        ...
        if let Some(cands) = index.get(&hash64(d)) {
            for &(w, i, j) in cands {
                ...
                let ok = pred_seg[w][i]
                    ...
                    .all(|((x, y), z)| (x ^ y) == *z);
                if ok {
                    found = Some((w, pred_pos[w][j].abs_diff(pred_pos[w][i]), pred_pos[w][i]));
                    break;
                }
            }
        }
        if let Some((w, sp, wpos)) = found {
            hits += 1;
            e.0 += 1;
            by_type.get_mut(gtype(&sg[gi])).unwrap().0 += 1;
            spans.push(sp);
            ...
            loc_frac.push(fr);
            if gtype(&sg[gi]) == "linear (CNOT/X)" {
                loc_frac_lin.push(fr);
            }
            *loc_block
                .entry(if w < 128 {
                    "data0(0-127)"
    ...
    println!(
        "=== LOCAL endpoint-flip match: source={} pred={} ===",
        source, pred
    );
    ...
    println!(
        "  gates matched                   : {hits} / {} ({:.3}%)",
        sg.len() - trivial,
        100.0 * hits as f64 / (sg.len() - trivial) as f64
    );
    ...
    if !spans.is_empty() {
        ...
        v.sort_unstable();
        println!(
            "  gate-span of matched pair       : min {} median {} max {}",
            v[0],
            v[v.len() / 2],
            v[v.len() - 1]
        );
    ...
    if !loc_frac.is_empty() {
        ...
        println!(
            "     in the last 10% of the circuit: {} / {} ({:.1}%)",
            late,
            v.len(),
            100.0 * late as f64 / v.len() as f64
        );
        if !loc_frac_lin.is_empty() {
            let mut l = loc_frac_lin.clone();
            l.sort_by(|a, b| a.partial_cmp(b).unwrap());
            let latel = l.iter().filter(|&&f| f > 0.9).count();
            println!(
                "     LINEAR-gate matches: median {:.3}, last 10%: {} / {} ({:.1}%)",
                l[l.len() / 2],
                latel,
                l.len(),
                100.0 * latel as f64 / l.len() as f64
            );
        }
        println!("  matched-pair predictor wire block:");
        let mut bk: Vec<&&str> = loc_block.keys().collect();
        bk.sort();
        for k in bk {
            println!("     {:<18} {}", k, loc_block[*k]);
        }
    }
