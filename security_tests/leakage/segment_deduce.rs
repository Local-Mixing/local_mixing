...
fn main() {
    ...
    let cuts: Vec<usize> = (0..=pred_cuts)
        .map(|i| (pg.len() as f64 * i as f64 / pred_cuts as f64).round() as usize)
        .collect();
    ...
    let mut collect =
        |wi: usize, xs: &[u128], lin: &mut Vec<u64>, seg: &mut Vec<u64>, sww: usize| {
            // predictor
    ...
    if !kept.is_empty() {
        ...
        let interior_only = kept
            .iter()
            .filter(|h| h.cut_lo > 0 && h.cut_hi < ncut - 1)
            .count();
        println!(
