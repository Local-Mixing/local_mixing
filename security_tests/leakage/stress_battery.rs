...
fn read_circuit(path: &str, fmt: &str) -> Vec<XGate> {
    match fmt {
        ...
        "mpmct1" => {
            read_mpmct(path)
                .unwrap_or_else(|_| panic!("read {path} (mpmct1)"))
                .0
        }
...
fn build_target(
    ...
    match kind {
        ...
        TargetKind::Par2 => {
            let (a, c) = pairs[t];
            (0..batches)
                .map(|batch| cs[batch][ri][a] ^ cs[batch][ri][c])
                .collect()
        }
        TargetKind::Traj => {
            let b = bits[t];
            ...
            (0..batches)
                .map(|batch| cs[batch][ri][b] ^ cs[batch][prev][b])
                .collect()
        }
    }
}
...
fn run_probe(
    ...
    for ri in 0..s.rows {
        for cj in 0..s.cols {
            let wires: Vec<Vec<u64>> = pred
                .iter()
                .map(|&k| (0..s.batches).map(|b| s.gs[b][cj][k]).collect())
                .collect();
...
fn census(g: &[XGate], nw: usize, band: &[usize], windows: &[usize]) -> Census {
    ...
    let w1share: Vec<f64> = (0..nw)
        .map(|w| {
            if writes[w] > 0 {
                w1writes[w] as f64 / writes[w] as f64
            } else {
                0.0
            }
        })
    ...
    let (band_write_auc, band_w1share_auc) = if band.is_empty() {
        ...
        (Some(auc(&|w| writes[w] as f64)), Some(auc(&|w| w1share[w])))
    };
...
fn main() {
    ...
    let and_set = if args.and_wires.is_empty() {
        Vec::new()
    } else {
        and_set
    };
    let band = parse_wire_list(&args.band, nw_g);
    let band = if args.band.is_empty() {
        Vec::new()
    } else {
        band
    };
    ...
    let decoys: Vec<Vec<XGate>> = if args.decoy.is_empty() {
        ...
        args.decoy
            .split(',')
            .map(|p| read_circuit(p.trim(), &args.c_format))
            .collect()
    };
    ...
    let floor = {
        ...
        for cj in (cols / 4..cols.saturating_sub(cols / 4)).step_by((cols / 8).max(1)) {
            let wires: Vec<Vec<u64>> = pred
                .iter()
                .map(|&k| (0..batches).map(|b| s.gs[b][cj][k]).collect())
                .collect();
    ...
    for (tname, kind) in probe_kinds {
        let (r1, r2) = run_probe(
            &s, &s.cs, kind, &bits, &pairs, &pred, &and_set, nbits, floor, &mut rng,
        );
    ...
    if let (Some(a), Some(b)) = (cen.band_write_auc, cen.band_w1share_auc) {
        println!(
            "    band localization AUC: write-count={a:.3}  width1-share={b:.3}  (0.5=hidden, 1.0=trivially found)"
        );
    }
...
impl Census {
    fn to_json(&self) -> String {
        ...
        format!(
            ...
            self.reads_med,
            self.reads_max,
            self.write_bimodality_gap,
            uw.join(","),
            opt(self.band_write_auc),
            opt(self.band_w1share_auc)
        )
    }
}
...
impl ProbeReport {
    fn to_json(&self) -> String {
        ...
        format!(
            ...
            self.feature,
            self.ridge.to_json(),
            f64_list(&self.decoy_prominence),
            contrast
        )
    }
}
...
impl Report {
    fn to_json(&self) -> String {
        ...
        format!(
            ...
            self.samples,
            self.floor,
            self.invariant_interior_max,
            self.invariant_interior_median,
            probes.join(","),
            self.census.to_json()
        )
    }
