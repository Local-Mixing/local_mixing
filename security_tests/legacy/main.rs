...
mod commands {
    pub use super::{compress, shoot, shuffle, ssg, sss};
    pub mod genran {
        pub fn run(m: &clap::ArgMatches) {
            super::super::finish(super::super::circuit::generate::run(m));
        }
    }
    pub mod evaluate {
        pub fn run(m: &clap::ArgMatches) {
            super::super::finish(super::super::circuit::evaluate::run(m));
        }
    }
    pub mod equal {
        pub fn run(m: &clap::ArgMatches) {
            super::super::finish(super::super::circuit::compare::run(m));
        }
    }
}
#[path = "../../src/commands/circuit/mod.rs"]
mod circuit;
pub mod compress;
pub mod shoot;
pub mod shuffle;
...
fn main() {
    let matches = Command::new("legacy_mixing")
        .about("Retained historical workflows for explicit comparison runs")
