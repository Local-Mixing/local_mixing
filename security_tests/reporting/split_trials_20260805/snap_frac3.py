...
REPO_ROOT = Path(os.environ.get("SECURITY_REPO_ROOT", Path(__file__).resolve().parents[3]))
REPORT_DIR = Path(os.environ.get("SECURITY_REPORT_DIR", REPO_ROOT / "reports" / "split_trials_20260805"))
...
def metrics(path):
    fams = {}
    n = 0
    with (REPORT_DIR / path).open() as f:
        it = iter(f)
        move = None
        for line in it:
...
paths = sorted(glob.glob("xp_r2_b3_c1_snap.mpmct1.mv*.mpmct1.state", root_dir=REPORT_DIR),
               key=lambda p: int(re.search(r"mv(\d+)", p).group(1)))
paths += [p for p in ["xp_r2_b3_c1_snap.state"] if glob.glob(p, root_dir=REPORT_DIR)]
rows = []
for p in paths:
    r = metrics(p)
    rows.append(r)
    print(p, r)
with (REPORT_DIR / "snap_frac3.csv").open("w", newline="") as f:
