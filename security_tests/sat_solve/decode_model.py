#!/usr/bin/env python3
"""Decode a DIMACS SAT model and verify the zero-slice preimage by circuit execution."""
...
def evaluate_mpmct1(path: Path, state: int) -> tuple[int, int, int]:
    with path.open("r", encoding="utf-8") as source:
        ...
        if wires <= 0 or expected_gates < 0:
            raise ValueError("invalid mpmct1 dimensions")
        gates = 0
...
def main() -> int:
    ...
    parser.add_argument("--circuit", type=Path, required=True, help="mpmct1 circuit used by zero_slice_to_cnf")
    parser.add_argument("--solver-output", type=Path, required=True, help="solver log containing s SATISFIABLE and v model lines")
    parser.add_argument("--n", type=int, required=True, help="logical input and target width")
    parser.add_argument("--output-start", type=int, help="first constrained output wire (default: n)")
    parser.add_argument("--target-hex", required=True, help="same little-endian target value used for CNF generation")
    parser.add_argument("--out", type=Path, required=True, help="new JSON verification report; an existing file is never overwritten")
    args = parser.parse_args()
    output_start = args.n if args.output_start is None else args.output_start
    if args.n <= 0 or output_start < 0:
        parser.error("n must be positive and output-start must be nonnegative")

    with args.circuit.open("r", encoding="utf-8") as source:
        header = source.readline().split()
    if len(header) != 3 or header[0] != "mpmct1":
        raise ValueError("invalid final-circuit header")
    total_wires = int(header[1])
    if args.n > total_wires or output_start + args.n > total_wires:
        raise ValueError("logical input or requested output range exceeds circuit width")
    ...
    if target < 0 or target & ~logical_mask:
        raise ValueError("target is wider than n")

    full_output, parsed_wires, gate_count = evaluate_mpmct1(args.circuit, full_input)
    logical_output = (full_output >> output_start) & logical_mask
    ...
    record: dict[str, object] = {
        ...
        "output_convention": f"target on wires {output_start}..{output_start + args.n - 1}",
    }
