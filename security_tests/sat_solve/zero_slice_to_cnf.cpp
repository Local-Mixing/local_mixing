...
int main(int argc, char **argv) {
  try {
    if (argc < 5 || (argc == 2 && std::string(argv[1]) == "--help")) {
      ...
      stream << "usage: zero_slice_to_cnf CIRCUIT.mpmct1 OUT.cnf N TARGET_HEX "
                "[--output-start WIRE] [--analyze-only]\n"
                "Find an N-bit input on wires [0,N), with all other input wires zero.\n"
                "Constrain N output bits starting at WIRE (default N) to TARGET_HEX.\n"
                "Wire numbering and bit order are little-endian; width need not be 4N.\n"
                "--analyze-only validates the circuit and reports formula size without writing.\n";
      return help ? 0 : 2;
    }
    const std::string circuit_path = argv[1];
    const std::string output_path = argv[2];
    if (circuit_path == output_path) throw std::runtime_error("CNF output must differ from circuit input");
    const int n = parse_nonnegative_int(argv[3], "logical width n");
    if (n == 0) throw std::runtime_error("logical width n must be positive");
    ...
    for (int index = 5; index < argc; ++index) {
      ...
      if (option == "--analyze-only") {
        analyze_only = true;
      } else if (option == "--output-start" && index + 1 < argc) {
        output_start = parse_nonnegative_int(argv[++index], "output start");
      } else {
    ...
    if (n > first.wires || output_start > first.wires - n) {
      throw std::runtime_error("logical input or requested output range exceeds circuit width");
    }
    ...
    output << "c zero-slice preimage: inputs " << n << ".." << (first.wires - 1)
           << " fixed to zero; outputs " << output_start << ".." << (output_start + n - 1)
    ...
    for (int bit = 0; bit < n; ++bit) {
      const int variable = state[static_cast<std::size_t>(output_start + bit)];
