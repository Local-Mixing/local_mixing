...
// Replacement lookups are served by the frozen store (src/db_mixing/frozen.rs):
...
fn feistal_middle_matches_once(
    ...
    if eval_wires <= 128 {
        ...
        let original_output =
            Gate::evaluate_index_list_128(u1024_low_u128(x), &original.gates) & mask;
        ...
        let original_output =
            Gate::evaluate_index_list_256(u1024_low_u256(x), &original.gates) & mask;
        let transformed_output =
        ...
        let original_output =
            Gate::evaluate_index_list_512(u1024_low_u512(x), &original.gates) & mask;
        let transformed_output =
...
fn print_sat_cone(label: &str, gates: &[[u16; 3]], cone_range: Option<(usize, usize)>) {
    ...
    if let Some(stats) =
        crate::db_mixing::sat_score::output_cone_stats(gates, output_start, output_bits)
    {
        println!(
...
pub fn main_shuffle_shoot_shuffle(
    ...
    let mut total_t = crate::db_mixing::transpositions::Transpositions {
    ...
    use crate::db_mixing::transpositions::{
    ...
    let track = crate::db_mixing::replace::track_survivors();
    ...
    let sidecar: Option<Vec<Tag>> = if track && !do_gadgetize && !do_feistalize {
        crate::db_mixing::replace::read_tags_sidecar(source)
    } else {
        None
    };
    let mut survivor_tags: Vec<Tag> = if !track {
        ...
        crate::db_mixing::replace::bump_litter_ids_past(&loaded);
        println!(
            "[tags] resumed {} gate tags from {}.tags (litter ids continue above prior max)",
            loaded.len(),
            source
        );
        crate::db_mixing::replace::gen_report("[tags] resumed state:", &loaded);
        loaded
    } else if crate::db_mixing::replace::gen_mode() {
    ...
    let samf_target = crate::db_mixing::replace::SAMF_TARGET.load(Relaxed);
    let mut m_eff = m;
    ...
    let grow_permille = crate::db_mixing::replace::GROW_THRESHOLD_PERMILLE.load(Relaxed);
    ...
    let compress_fraction_permille =
        crate::db_mixing::replace::COMPRESS_FRACTION_PERMILLE.load(Relaxed);
    ...
    // is requested.
    let target_size = crate::db_mixing::replace::TARGET_SIZE.load(Relaxed);
    ...
    loop {
        ...
        if stage_d && i >= STAGE_D_MAX_STAGES {
            println!(
                "[stage-D] reached safety cap of {} stages; stopping",
                STAGE_D_MAX_STAGES
            );
            break;
        }
        if stage_d {
            ...
            crate::db_mixing::replace::SHOOT_SIZE_CAP
        ...
        crate::db_mixing::replace::REC_ROUND.store(i + 1, std::sync::atomic::Ordering::Relaxed);
        if sat_global_mix_enabled() && i % sat_global_mix_every() == 0 {
            ...
            if mix_m > 0 {
                ...
                insert_wire_m_samfs_every_x(&mut circuit, n, mix_m, mix_x, db, true, true);
                println!(
        ...
        if egg {
            ...
            use crate::db_mixing::transpositions::shuffled_shoot_then_samf_core;
            ...
            use crate::db_mixing::transpositions::shuffled_shoot_then_samf;
        ...
        if single_end && i == rounds - 1 {
            use crate::db_mixing::transpositions::apply_unsamf;
            apply_unsamf(
                &mut circuit.gates,
                &total_t,
                &total_neg,
        ...
        let stage_d_compress_target =
            if stage_d && target_size > 0 && compress_fraction_permille > 0 {
                let t =
                    ((target_size as u128 * compress_fraction_permille as u128) / 1000) as usize;
                println!(
        ...
        // Record + print this round's SAMF stats (deltas from the previous round).
        {
            ...
            let unclean = crate::db_mixing::transpositions::UNCLEAN_EXPANSIONS.load(Relaxed);
            println!(
            ...
            if crate::db_mixing::replace::litter_rules() && crate::db_mixing::replace::gen_mode() {
                crate::db_mixing::replace::litter_report("    [litter] cumulative:");
            }
        ...
        crate::circuit::cancel_adjacent_duplicates(
            &mut circuit.gates,
        ...
        }
        {
            ...
            if track {
                crate::db_mixing::replace::write_tags_sidecar(&round_path, &survivor_tags);
            }
            if equality_check {
        ...
        if stage_d {
            ...
            let permille = crate::db_mixing::replace::MIN_GEN_PERMILLE.load(Relaxed);
    ...
    crate::db_mixing::replace::SHOOT_SIZE_CAP.store(0, std::sync::atomic::Ordering::Relaxed);

    if track && crate::db_mixing::replace::gen_mode() {
        ...
        let max_gen = survivor_tags
            .iter()
            .map(|t| t.generation())
            .max()
            .unwrap_or(0);
        let min_gen = survivor_tags
            .iter()
            .map(|t| t.generation())
            .min()
            .unwrap_or(0);
        ...
        let median_gen = if sorted.is_empty() {
            0
        } else {
            sorted[sorted.len() / 2]
        };
    ...
    println!(
        ...
        crate::circuit::OVERSIZED_CANON_SKIPS.load(std::sync::atomic::Ordering::Relaxed)
    );
    println!(
        "Forced pseudo-collisions (no real collision): {}",
        crate::db_mixing::replace::FORCED_COLLISIONS.load(std::sync::atomic::Ordering::Relaxed)
    );
    ...
    if track {
        crate::db_mixing::replace::write_tags_sidecar(save, &survivor_tags);
    }
...
#[cfg(test)]
#[path = "../../../tests/unit/legacy/db_mixing/main_mix/opt_equiv_tests.rs"]
mod opt_equiv_tests;
