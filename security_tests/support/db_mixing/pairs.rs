...
use crate::{
    circuit::{CircuitSeq, random_circuit},
    db_mixing::frozen::FrozenDb,
    db_mixing::sat_score::{
...
///
/// The value rides out as the `Arc<[u8]>` the lookup cache already holds: a
/// curated value can be many megabytes, and every consumer only reads
/// `value.len()` and `&value[..]`, both of which `Arc<[u8]>` provides by Deref.
fn frozen_lookup(
    sub: &CircuitSeq,
    db: &FrozenDb,
    use_curated: bool,
    ...
    if use_regular && negated_inputs.is_empty() {
        ...
        match min_dir_lookup_mode() {
            ...
            mode => {
                let fwd_polys = fwd_polys.expect(
                    "min-dir compare requires forward polys (computed above for this mode)",
                );
                ...
                if let Some(v) = cached_db_get(db, LOOKUP_NS_SHARD, &min_key) {
                    return Some((v, min_order, min_reversed, used));
                }
                if mode == MinDirLookup::Validate {
                    ...
                    if let Some(v) = cached_db_get(db, LOOKUP_NS_SHARD, &alt_key) {
                        ...
                        return Some((v, alt_order, alt_reversed, used));
                    }
                }
            }
        }
...
pub fn expand_curated_db_neg(
    ...
    let mut best: Vec<CircuitSeq> = if crate::db_mixing::replace::gen_mode() {
        ...
        crate::db_mixing::replace::gen_select_best(candidates)
        ...
        // used_wires_len is the allocation-free twin of used_wires().len(); the
        // score is evaluated twice per candidate (max pass, then filter pass),
        // so materializing the sorted wire list here cost two Vecs per candidate.
        let score = |c: &CircuitSeq| c.gates.len() + c.used_wires_len();
