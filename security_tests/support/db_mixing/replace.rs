...
use crate::db_mixing::util::split_into_random_chunk_ranges;
use rand::Rng;
...
pub fn litter_report(prefix: &str) {
    ...
    let avg_distinct = if c[2] > 0 {
        c[4] as f64 / c[2] as f64
    } else {
        0.0
    };
    println!(
...
pub fn read_tags_sidecar(circuit_path: &str) -> Option<Vec<Tag>> {
    ...
    let tags: Option<Vec<Tag>> = it
        .map(|s| u64::from_str_radix(s, 16).ok().map(Tag))
        .collect();
    let tags = tags?;
    if tags.len() != count {
        eprintln!(
            "[tags] {}: expected {} tags, found {} — ignoring",
            path,
            count,
            tags.len()
        );
...
#[cfg(test)]
fn derivative_witness(gates: &[[u16; 3]], nw: usize, dirs: &[Vec<bool>], base: &[bool]) -> bool {
    let m = dirs.len();
...
pub fn cand_features(
    ...
    left: &[[u16; 3]],
    right: &[[u16; 3]],
) -> crate::db_mixing::ranking::CandFeatures {
    use crate::circuit::Gate;
    use crate::db_mixing::ranking::CandFeatures;
    let thr = MIN_MEDIAN_LEEWAY.load(Ordering::Relaxed);
    let n = window.len();
...
// Compatibility exports for retained comparison tools.
pub use crate::db_mixing::lookup_cache::{
    LOOKUP_CACHE_HITS, LOOKUP_CACHE_QUERIES, MIN_DIR_VALIDATE_PROBES, MIN_DIR_VIOLATIONS,
};
pub(crate) use crate::db_mixing::lookup_cache::{
    LOOKUP_NS_CURATED, LOOKUP_NS_SHARD, MinDirLookup, cached_db_get, min_dir_lookup_mode,
};
    ...
    for _ in 0..trials {
        ...
        if sub.gates.len() == 2 {
            match pair_mode {
                ExpandPairMode::Curated => {
                    use crate::db_mixing::pairs::expand_curated_db;
        ...
        if !fwd_high {
            ...
            if let Some(fwd_key) = fwd_key {
                ...
                let fwd_result = crate::db_mixing::replace::cached_db_get(
                    db,
                    crate::db_mixing::replace::LOOKUP_NS_SHARD,
                    &fwd_key,
                );
        ...
        if matched.is_none() {
            ...
            if rev_high {
                ...
                if let Some(rev_key) = rev_key {
                    ...
                    let rev_result = crate::db_mixing::replace::cached_db_get(
                        db,
                        crate::db_mixing::replace::LOOKUP_NS_SHARD,
                        &rev_key,
                    );
...
pub fn compress_db(
    ...
    crate::circuit::cancel_adjacent_duplicates(&mut compressed.gates, track.then(|| &mut *tags));
    ...
    for _ in 0..trials {
        ...
        let repl: CircuitSeq = if gen_mode() {
            ...
            let feats: Vec<crate::db_mixing::ranking::CandFeatures> = min_set
                .iter()
                .map(|cc| cand_features(&cc.gates, left, right))
                .collect();
            let order = crate::db_mixing::ranking::incoming().order(&feats);
            ...
            let mut best: Vec<CircuitSeq> = if sat_scoring_enabled() {
                ...
                let scored: Vec<(f64, CircuitSeq)> = candidates
                    ...
                    .filter_map(|(idx, candidate)| {
                        let score_n = candidate.max_wire() + 1;
                        let sat_score =
                            score_subcircuit(&candidate.gates, score_n, seed ^ idx as u64);
                        if sat_cone_aware_enabled()
                            && sat_score.output_cone_fraction < sat_cone_min_fraction()
                        {
                            return None;
...
pub fn compress_big_ancillas(
    ...
    crate::circuit::cancel_adjacent_duplicates(&mut circuit.gates, track.then(|| &mut *tags));
...
pub fn print_compress_timers() {
    use crate::circuit::{CANON4_RULE_L_BRANCHES, CANON4_RULE_L_CALLS, CANON4_RULE_L_TIME};
    use crate::db_mixing::transpositions::{SAMF_COMPRESSIONS_FAILED, SAMF_COMPRESSIONS_MADE};
    ...
    if std::env::var("BENCH_CANON").is_ok() {
        use crate::circuit::{CANON_BENCH_CALLS, CANON4_CORE_TIME, POLYCANON_CORE_TIME};
...
#[cfg(test)]
#[path = "../../../tests/unit/legacy/db_mixing/replace/float_tests.rs"]
mod float_tests;

#[cfg(test)]
#[path = "../../../tests/unit/legacy/db_mixing/replace/degree_filter_tests.rs"]
