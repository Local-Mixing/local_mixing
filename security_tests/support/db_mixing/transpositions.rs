...
fn integrate_samf_compressed(
    ...
    use_curated: bool,
    use_regular: bool,
    tags: &mut Vec<crate::db_mixing::replace::Tag>,
) {
    use crate::db_mixing::pairs::{compress_curated_db, find_any_replacement_db};
    ...
    if !use_curated && !use_regular {
        ...
        if track {
            ...
            let nt = crate::db_mixing::replace::new_gate_tag(
                &tags[tags.len() - k..],
                gates.len() - tags.len(),
            );
    ...
    if track {
        ...
        let nt =
            crate::db_mixing::replace::new_gate_tag(&tags[ctx_start..], gates.len() - ctx_start);
...
fn insert_m_samfs_core(
    ...
    m: usize,
    x: usize,
    input_tags: &[crate::db_mixing::replace::Tag],
) -> (
    Vec<[u16; 3]>,
    Transpositions,
    Vec<u8>,
    Vec<crate::db_mixing::replace::Tag>,
) {
    ...
    let mut wmap = WireMap::identity(n);
    let mut gates: Vec<[u16; 3]> = Vec::new();
    let mut out_tags: Vec<crate::db_mixing::replace::Tag> = Vec::new();
    let mut negation_mask = vec![0u8; n];
    for (i, gate) in input.iter().enumerate() {
        ...
        if track {
            ...
            out_tags.resize(
                gates.len() - 1,
                crate::db_mixing::replace::new_gate_tag(
...
fn shuffled_shooting_game_core(
    ...
    use crate::db_mixing::pairs::{compress_curated_db, expand_curated_db_neg};
    ...
    while materialized_shot.is_some() || !remaining.is_empty() {
        ...
        let forced_collision =
            !has_collision && crate::db_mixing::replace::gen_mode() && !passed.is_empty();
        if forced_collision {
            ...
            has_collision = true;
            crate::db_mixing::replace::FORCED_COLLISIONS
        ...
        let replaced = 'try_replace: {
            ...
            let (consumed_indices, window, expansion, picked_negated, picked_full_litter): (
                ...
                if crate::db_mixing::replace::gen_mode() {
                    ...
                    if !rg.is_empty() {
                        ...
                        let reach1: Vec<usize> =
                            (1..rg.len()).filter(|&j| reach(j, NO_EX, NO_EX)).collect();
                        for &j in &reach1 {
                ...
                let order: Vec<usize> = if crate::db_mixing::replace::gen_mode() {
                    let left = &output[output.len().saturating_sub(FEATURE_CTX)..];
                    let feats: Vec<crate::db_mixing::ranking::CandFeatures> = candidates
                        .iter()
                        .map(|(idxs, win)| {
                            ...
                            let right: Vec<[u16; 3]> = (0..rg.len())
                                ...
                                .map(|m| rg[m])
                                .collect();
                            crate::db_mixing::replace::cand_features(win, left, &right)
                        })
                        .collect();
                    ...
                    let mut ord = crate::db_mixing::ranking::outgoing().order(&feats);
                ...
                let litter_active = track
                    && crate::db_mixing::replace::gen_mode()
                    && crate::db_mixing::replace::litter_rules();
                ...
                let order: Vec<usize> = if litter_active {
                    litter_stats = candidates
                        ...
                        .map(|(idxs, _)| {
                            ...
                            crate::db_mixing::replace::window_litter_stats(&wt)
                        })
                        .collect();
                    let mut ord = order;
                    ord.sort_by_key(|&i| {
                ...
                let gen_m = crate::db_mixing::replace::gen_mode();
                        ...
                        expand_curated_db_neg(win, n, db, use_curated, use_regular, &neg)
                    {
                        if e.len() >= 3 {
                            ...
                            if litter_active && !full {
                                ...
                                if distinct >= 2 {
                                    crate::db_mixing::replace::LITTER_TIER1
                                        .fetch_add(1, Ordering::Relaxed);
                                    crate::db_mixing::replace::LITTER_DISTINCT_SUM
                                        .fetch_add(distinct, Ordering::Relaxed);
                                } else {
                                    crate::db_mixing::replace::LITTER_TIER2
                                        .fetch_add(1, Ordering::Relaxed);
                                }
                            }
            ...
            if picked_negated.is_empty() && crate::db_mixing::replace::verify_db_hits() {
                let win_c = crate::circuit::CircuitSeq {
                    gates: window.clone(),
                };
                let exp_c = crate::circuit::CircuitSeq {
            ...
            let event_gen: u32 = if track && crate::db_mixing::replace::gen_mode() {
                let mut win: Vec<Tag> = Vec::with_capacity(consumed);
                win.push(shot_tag);
                win.extend(consumed_indices.iter().map(|&idx| remaining_tags[idx]));
                crate::db_mixing::replace::median_floor_gen(&win).saturating_add(1)
            } else {
                0
            };
            ...
            let cone_aware = crate::db_mixing::sat_score::sat_cone_aware_enabled();
            let pair_attempts = if cone_aware {
                crate::db_mixing::sat_score::sat_hidden_samf_candidates()
            } else {
                samf_hide_pairs()
            };
                    ...
                    if let Some(repl) = compress_curated_db(&hide_window, n, db, false, use_regular)
                    {
                        ...
                        if samf_hidden {
                            let score = if cone_aware {
                                let sat_score = crate::db_mixing::sat_score::score_subcircuit(
                                    &repl,
                                    n,
                                    crate::db_mixing::sat_score::sat_score_seed()
                                        ^ ((pair_idx as u64) << 8)
                                        ^ neg_type as u64,
                                );
                                crate::db_mixing::sat_score::compression_selection_score(&sat_score)
                            } else {
                                0.0
                            };
            ...
            if picked_full_litter && tuck.is_none() && picked_negated.is_empty() {
                crate::db_mixing::replace::LITTER_ABORT_NO_SAMF.fetch_add(1, Ordering::Relaxed);
                break 'try_replace false;
            }
            if picked_full_litter && tuck.is_some() {
                crate::db_mixing::replace::LITTER_SAMF_LICENSED.fetch_add(1, Ordering::Relaxed);
            }
            ...
            crate::db_mixing::replace::record_expansion(
                consumed,
                expansion.len(),
                before_wires,
            ...
            let event_tag = if track && crate::db_mixing::replace::gen_mode() {
            ...
            if crate::db_mixing::replace::record_enabled() {
                let rec_end = output.len();
                let rs = rec_start.min(rec_end);
                let ws: Vec<u16> = output[rs..rec_end].iter().flatten().copied().collect();
                crate::db_mixing::replace::record_replacement(
                    "expand",
                    crate::db_mixing::replace::REC_PASS.load(std::sync::atomic::Ordering::Relaxed),
                    rs,
                    rec_end,
                    consumed,
                    &ws,
    ...
    while let Some(g) = remaining.pop_front() {
        ...
        if track {
            ...
            output_tags.resize(
                output.len() - 1,
                crate::db_mixing::replace::new_gate_tag(std::slice::from_ref(&gt), new_count),
            );
            output_tags.push(gt);
        }
    }
...
pub fn shuffled_shoot_then_samf_core(
    ...
    if crate::db_mixing::replace::gen_mode() && track {
        use crate::db_mixing::segcircuit::{SamfLedger, SamfTail, SegCircuit};
        use std::sync::atomic::Ordering::Relaxed;
        const WINDOW_CAP: usize = 8192; // max gates a single short pass spans
        let mut rng = rand::rng();
        let min_gen_target = crate::db_mixing::replace::MIN_GEN.load(Relaxed) as u32;
        let pass_length = crate::db_mixing::replace::PASS_LENGTH.load(Relaxed);
        let max_passes = crate::db_mixing::replace::MAX_PASSES.load(Relaxed);
        let min_gen_permille = crate::db_mixing::replace::MIN_GEN_PERMILLE.load(Relaxed);
        // Stage D: pause shooting (return to the driver to compress) once the working circuit
        // reaches this many gates. 0 = no cap.
        let shoot_size_cap = crate::db_mixing::replace::SHOOT_SIZE_CAP.load(Relaxed);
        ...
        let global_check = |sc: &SegCircuit,
         ...
         -> bool {
            ...
            let net_perm = crate::circuit::Permutation {
        ...
        loop {
            ...
            crate::db_mixing::replace::REC_PASS.store(pass_idx + 1, Relaxed);
    ...
    for pass in 0..passes {
        crate::db_mixing::replace::REC_PASS.store(pass + 1, std::sync::atomic::Ordering::Relaxed);
        ...
        if track {
            ...
            if (pass + 1) % every == 0 || pass + 1 == passes {
                if crate::db_mixing::replace::gen_mode() {
                    ...
                    let alive = tags
                        .iter()
                        .filter(|&&t| t != crate::db_mixing::replace::Tag::NEW)
                        .count();
                    println!(
...
fn apply_ledger(
    gates: &[[u16; 3]],
    tags: &[Tag],
    entries: &[(usize, crate::db_mixing::segcircuit::SamfTail)],
    n: usize,
) -> (
    Vec<[u16; 3]>,
    Vec<Tag>,
    crate::db_mixing::segcircuit::SamfTail,
) {
    use crate::db_mixing::segcircuit::SamfTail;
    let track = !tags.is_empty();
    // Running permutation as a direct map (wmap.relabel == what the reference t_list would
    // evaluate) — the historical apply_ledger_ref test oracle (since removed) applied the list-of-swaps
    // fold this replaces.
    let mut wmap = WireMap::identity(n);
    ...
    let fold = |wmap: &mut WireMap, mask: &mut Vec<u8>, e: &SamfTail| {
    ...
    for (i, &g) in gates.iter().enumerate() {
        ...
        if track {
            ...
            if added > 1 {
                let nt = crate::db_mixing::replace::new_gate_tag(&[tags[i]], added - 1);
...
#[cfg(test)]
#[path = "../../../tests/unit/legacy/db_mixing/transpositions/reversed_samf_tests.rs"]
mod reversed_samf_tests;

#[cfg(test)]
#[path = "../../../tests/unit/legacy/db_mixing/transpositions/unsamf_scale_tests.rs"]
