//! Historical gadget families retained for optional security comparisons.

use super::guards::try_slice_zero_block_dims;
...
use super::shared::random_wire_except;
...
pub use crate::stages::preprocessing::slice_guards::SLICE_ZERO_CCNOT_GATES_PER_WIRE;
...
#[cfg(test)]
#[path = "../../../tests/stages/preprocessing/legacy_gadgets_feistal_tests.rs"]
mod feistal_tests;

#[cfg(test)]
#[path = "../../../tests/stages/preprocessing/legacy_gadgets_feistal_property_tests.rs"]
mod feistal_property_tests;

#[cfg(test)]
#[path = "../../../tests/stages/preprocessing/legacy_gadgets_feistal_structural_tests.rs"]
mod feistal_structural_tests;

#[cfg(test)]
#[path = "../../../tests/stages/preprocessing/legacy_gadgets_feistal_32_wire_tests.rs"]
mod feistal_32_wire_tests;

#[cfg(test)]
#[path = "../../../tests/stages/preprocessing/legacy_gadgets_slice_zero_random_large_wire_tests.rs"]
mod slice_zero_random_large_wire_tests;

#[cfg(test)]
#[path = "../../../tests/stages/preprocessing/legacy_gadgets_feistal_fixed_point_n_tests.rs"]
...
impl ProdLedger {
    ...
    fn rewrite_var(
        ...
        let draw = |rng: &mut dyn rand::RngCore, allow_carrier: bool| -> u16 {
            let use_carrier =
                allow_carrier && state.n > 0 && (rng.next_u32() as usize % 100) < refill_data;
            if use_carrier {
                for _ in 0..64 {
        ...
        for value in lo..state.n {
            ...
            if self.consts[value] {
                ...
                let u = if self.tail_band_helpers && !self.loc.is_empty() {
                    ...
                    loop {
                        let w = self.loc[rng.random_range(0..self.loc.len())] as usize;
    ...
    loop {
        ...
        for i in 0..n {
            ...
            for j in (i + 1)..(i + 1 + WINDOW).min(n) {
                ...
                let gj = &gates[j];
                if !drop[j]
                    && (gj.target == gi.target || gj.reads(gi.target) || gi.reads(gj.target))
                {
                    break;
                }
            }
...
fn emit_nswitch(p: usize, z: usize, q: usize, out: &mut Vec<XGate>) {
    debug_assert!(
        p != q && z != p && z != q,
        "NSWITCH needs three distinct wires"
    );
...
pub fn gadgetize_drip_layered(
    ...
    for layer in &layers {
        ...
        for key in keys {
            ...
            let singleton = if idxs.len() % 2 == 1 {
                idxs.pop()
            } else {
                None
            };
            if idxs.len() >= 2 {
                let buses: Vec<Vec<u16>> =
                    idxs.iter().map(|&gi| rail_vector(&source[gi])).collect();
                let k = buses.len();
...
#[cfg(test)]
#[path = "../../../tests/stages/preprocessing/legacy_gadgets_drip_tests.rs"]
mod drip_tests;
...
#[cfg(test)]
#[path = "../../../tests/stages/preprocessing/legacy_gadgets_cnot_gadget_tests.rs"]
mod cnot_gadget_tests;

#[cfg(test)]
include!("../../../tests/stages/preprocessing/legacy_gadgets_helpers.rs");
