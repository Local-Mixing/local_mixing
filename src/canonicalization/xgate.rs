...
pub fn xgates_to_polynomial(
    ...
    for gate in gates {
        ...
        let old_target_terms = polys[target].len();
        ...
        let new_target_terms = polys[target].len();
        let new_total = match total_terms
            .checked_sub(old_target_terms)
            .and_then(|n| n.checked_add(new_target_terms))
        {
            Some(total) => total,
            None => {
                // `check_total` reproduces the original overflow error.
                check_total(&polys, budget)?;
                unreachable!("polynomial term total overflow passed check_total")
            }
        };
        if new_total > budget.max_total_terms {
            // Preserve the historical error's exact `attempted` value on the
            // uncommon failing path; successful gates avoid rescanning every
            // wire polynomial.
            check_total(&polys, budget)?;
        }
        total_terms = new_total;
    }
    Ok(polys)
}
...
/// Number of distinct wires touched by `gates`, without materializing the
/// sorted wire list. Equals `xgate_used_wires(gates).len()`.
///
/// The XGate twin of `CircuitSeq::used_wires_len` (circuit.rs): the span guards
/// on the DB descent call this on every rung purely to compare against
/// `db_max_span`, and when the guard fires the discarded Vec was the whole cost
/// of the rung. Wires at or above 1024 fall back to the exact sorted path.
pub fn xgate_used_wires_len(gates: &[XGate]) -> usize {
    let mut words = [0u64; 16];
    for gate in gates {
        if gate.target >= 1024 || gate.ctrls.iter().any(|&(wire, _)| wire >= 1024) {
            return xgate_used_wires(gates).len();
        }
        words[(gate.target >> 6) as usize] |= 1u64 << (gate.target & 63);
        for &(wire, _) in &gate.ctrls {
            words[(wire >> 6) as usize] |= 1u64 << (wire & 63);
        }
    }
...
/// Explicit limits for composing and canonicalizing a heterogeneous window.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub struct XGateCanonicalizationOptions {
    pub budget: XPolyBudget,
    /// Zero accepts every ANF degree.
...
pub fn canonicalize_xgates_single_with_options(
    gates: &[XGate],
    reversed: bool,
    options: &XGateCanonicalizationOptions,
) -> Result<CanonicalXPolys, XPolyError> {
    ...
    let (polys, order) = compose_and_canonicalize(
        &dense,
        used_wires.len(),
        options.budget,
        options.max_degree,
        Some(&options.canonicalization),
    )?;
    Ok(CanonicalXPolys {
        polys,
        order,
...
pub fn canonicalize_xgates_single_capped(
    ...
    let Some(cache) = xpoly_canon_cache() else {
        let (polys, order) = compose_and_canonicalize(&dense, num_wires, budget, max_degree, None)?;
        return Ok(CanonicalXPolys {
            polys,
            order,
            used_wires,
    ...
    let computed = compose_and_canonicalize(&dense, num_wires, budget, max_degree, None);
    ...
    // Build the shared entry once and hand the caller a copy taken from it.
    // The previous form cloned `computed` for the cache and then moved the
    // original out, so every miss paid two deep copies of the polynomial vector
    // (1 + num_wires allocations each) where one will do. The cached bytes and
    // the returned value are unchanged.
    let entry = std::sync::Arc::new(computed);
...
fn compose_and_canonicalize(
    ...
    let canon_res = canonicalize_polys_4_using(polys, true, options);
...
pub static XPOLY_CANON_CACHE_QUERIES: std::sync::atomic::AtomicU64 =
    std::sync::atomic::AtomicU64::new(0);
static XPOLY_CANON_CACHE_BYTES: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(0);
...
#[cfg(test)]
#[path = "../../tests/unit/canonicalization/xgate/tests.rs"]
mod tests;
