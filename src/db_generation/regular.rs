...
#[cfg(test)]
#[path = "regular_validation_tests.rs"]
...
fn decode_rocks_entry(key: &[u8], value: &[u8]) -> Result<Vec<CircuitSeq>, String> {
    ...
    for chunk in &iter.chunks(chunk_size) {
        ...
        let entries = chunk
            .map(|item| {
                ...
                if let Some(circuit) = circuits.iter().find(|circuit| circuit.gates.len() != m - 1)
                {
                    return Err(format!(
