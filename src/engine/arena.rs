...
impl Arena {
    ...
    /// Traverse linked node ids in circuit order without allocating a
    /// temporary id vector. Callers that mutate the arena must continue to use
    /// [`Self::ids_in_order`] so their traversal is insulated from relinking.
    pub fn ids_in_order_iter(&self) -> impl Iterator<Item = u32> + '_ {
        std::iter::successors((self.head != NIL).then_some(self.head), |&id| {
