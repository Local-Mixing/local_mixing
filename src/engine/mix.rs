...
fn reference_db() -> Option<&'static crate::db_mixing::frozen::FrozenDb> {
    static REF: std::sync::OnceLock<Option<crate::db_mixing::frozen::FrozenDb>> =
        std::sync::OnceLock::new();
    REF.get_or_init(|| {
        let dir = std::env::var("FROZEN_REF_DIR").ok()?;
        println!("[fmix] class-attribution reference store: {dir}");
        Some(crate::db_mixing::frozen::FrozenDb::open(&dir, None))
    })
    .as_ref()
}
