// Shared fmix state and randomized equivalence-walk machinery for GSS Phase A
// and postprocessing. Production Stage-4 splitting and Stage-5 crossing live in
// postprocessing::{splitting,cross_walk} and operate on this shared Mixer. The
// objective is NOT size —
...
use crate::db_mixing::db_replace::{DbMode, DegreeGuard, db_replace};
...
/// mutation originates in Phase A or the crossing walk.
...
impl Mixer {
    ...
    fn db_attempt_inner(&mut self, mode: DbMode) -> bool {
            ...
            for &(cur_armed, reg_fb) in passes {
                ...
                loop {
                    ...
                    if self.params.db_max_span > 0
                        && super::xpoly::xgate_used_wires_len(prefix) > self.params.db_max_span
                    {
        ...
        if self.params.db_max_span > 0
            && super::xpoly::xgate_used_wires_len(&window) > self.params.db_max_span
        {
            self.counters.db_span_skips += 1;
            let k = self.db_last_len;
    ...
    fn try_db_splice_curated(
        ...
        if self.params.db_verify {
            ...
            let _vok = if support.len() > 24 {
                match crate::db_mixing::db_replace::polys_equivalent(
                    window,
                    &replacement,
                    self.db_budget,
                ) {
...
mod mix_tests {
    ...
    fn conj_wake_is_the_exact_conjugate() {
        ...
        for i in 0..6000 {
            ...
            let u =
                XGate::conj(tu, [(xw, rng.random_bool(0.5)), (yw, rng.random_bool(0.5))]).unwrap();
