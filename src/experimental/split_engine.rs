// Experimental standalone float-and-split episode loop.
//
...
impl Engine {
    pub fn new(gates: Vec<XGate>, params: Params) -> Engine {
        let num_wires = crate::circuit::xgate::max_wire(&gates) as usize + 1;
    ...
    fn maybe_dump(&mut self) {
        ...
        match crate::engine::format::write_mpmct(&tmp, &gates, self.num_wires) {
            Ok(()) => {
    ...
    pub fn global_check(&mut self) {
        ...
        for _ in 0..batches {
            ...
            crate::circuit::xgate::eval_lanes(&self.original, &mut st_orig);
