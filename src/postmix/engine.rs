...
use super::source::{UNKNOWN_SOURCE, merge_source};
...
mod source_tracking_tests {
    use super::*;
    use crate::postmix::source::MIXED_SOURCE;

    #[test]
    fn pair_rewrite_keeps_intact_parent_and_taints_dependent_piece() {
        let g = XGate::from_g57([0, 1, 2]);
        let h = XGate::from_g57([3, 4, 5]);
        let dependent = XGate::conj(0, [(1, true)]).unwrap();
        let mut engine = Engine::new(vec![g, h.clone()], Params::default());
        let placed = engine.splice_pair(
            0,
            1,
            Dir::R,
            vec![(h, Role::CollidingIntact), (dependent, Role::ShotPiece)],
            RuleKind::R1,
        );
        let marks: Vec<u32> = placed.iter().map(|&(id, _)| engine.source_of(id)).collect();
        assert_eq!(marks, vec![1, MIXED_SOURCE]);
    }

    #[test]
    fn presplit_pieces_retain_their_single_parent() {
        let g = XGate::from_g57([0, 1, 2]);
        let mut engine = Engine::new(vec![g], Params::default());
        let ids = engine.splice_replace_one(
            0,
            vec![
...
pub struct Engine {
    ...
    dumps_written: u64,
    original: Vec<XGate>,
    // Source-parent mark for every arena node id.  Arena ids are stable while
    // linked and may be recycled after a splice, so every insertion overwrites
...
impl Engine {
    ...
    fn trace_full(&self) -> bool {
        self.trace
            .as_ref()
            .is_some_and(|t| t.len() > self.trace_cap)
    }
        ...
        let gates = self
            ...
            .map(|&id| {
                ...
                (
                    g.target,
                    g.comp as u8,
                    g.ctrls.iter().map(|&(w, p)| (w, p as u8)).collect(),
                )
            })
            .collect();
        self.trace.as_mut().unwrap().push(TraceStep {
            kind,
            label,
            gates,
            moves,
    ...
    fn select_start(&mut self) -> Option<(u32, Dir)> {
        ...
        for _ in 0..self.params.candidates {
            ...
            let slot = if self.arena.gate(id).comp {
                &mut best_g57
            } else {
                &mut best_other
            };
    ...
    #[inline]
    fn source_of(&self, id: u32) -> u32 {
        self.source_marks[id as usize]
    }

    #[inline]
    fn set_source(&mut self, id: u32, mark: u32) {
        let need = id as usize + 1;
        if self.source_marks.len() < need {
            self.source_marks.resize(need, UNKNOWN_SOURCE);
        }
        self.source_marks[id as usize] = mark;
    }

    /// Source marks in exactly the same linked-list order as `arena.to_vec()`.
    pub fn source_marks_in_order(&self) -> Vec<u32> {
        self.arena
            .ids_in_order()
            .into_iter()
            .map(|id| self.source_of(id))
            .collect()
    }

    pub fn num_source_parents(&self) -> usize {
        self.original.len()
    }
    ...
    fn splice_pair(
        ...
        let g_source = self.source_of(g_id);
        let h_source = self.source_of(h_id);
        let mixed_source = merge_source(g_source, h_source);
        let first = match dir {
            Dir::R => g_id,
            Dir::L => h_id,
        };
        ...
        for (gate, role) in emitted {
            let source = match role {
                Role::CollidingIntact => h_source,
                // R2 passes the shot gate itself intact.  In R1/R3 its shot
                // residues depend on the colliding gate and are therefore
                // interaction-tainted even if their final algebra simplifies.
                Role::ShotPiece if kind == RuleKind::R2 => g_source,
                Role::ShotPiece | Role::CollidingPiece | Role::Core => mixed_source,
            };
    ...
    fn episode(&mut self) -> bool {
        ...
        while let Some((id, stamp, dir)) = wl.pop_front() {
            ...
            let pre_idx = if self.trace.is_some() {
            ...
            match rules::cross(&g, &h, self.params.k_max, &mut self.rng) {
                ...
                Outcome::Rewrite { seq, kind, dropped } => {
                    ...
                    let placed = self.splice_pair(id, h_id, dir, seq, kind);
                    if self.trace.is_some() {
                        let (kind_s, verb) = match kind {
                            RuleKind::R1 => (
                                "r1",
                                "split into the ladder and crossed; colliding gate intact",
                            ),
                            RuleKind::R2 => (
                                "r2",
                                "passed intact; colliding gate split and pushed the other way",
                            ),
                            RuleKind::R3 => (
                                "r3",
                                "mutual collision: stay-behind core wedged, rest crossed",
                            ),
                        };
    ...
    fn float_uniform(&mut self, id: u32) -> (usize, Option<(usize, usize)>) {
        ...
        let from = if self.trace.is_some() {
            self.arena.index_of(id)
        } else {
            0
        };
        ...
        let ft = if self.trace.is_some() {
            Some((from, self.arena.index_of(id)))
        } else {
            None
        };
        (k, ft)
    ...
    fn scatter(&mut self, ids: &[u32], what: &'static str) {
        ...
        if moved > 0 && self.trace.is_some() {
            let ids_now: Vec<u32> = ids
                .iter()
                .copied()
                .filter(|&i| self.arena.is_linked(i))
                .collect();
            self.emit_trace(
                "scatter",
