...
pub enum Outcome {
    ...
    Rewrite { seq: Vec<(XGate, Role)>, kind: RuleKind, dropped: usize },
...
pub fn cross(g: &XGate, h: &XGate, k_max: usize, rng: &mut impl Rng) -> Outcome {
    assert!(!g.comp, "shot gate must be a conjunction (pre-split g57 shots first)");
    let a = g.target;
    let b = h.target;
    if a == b {
...
fn r1(g: &XGate, h: &XGate, k_max: usize, rng: &mut impl Rng) -> Outcome {
    ...
    match XGate::conj(a, [(b, pb ^ flip_full)].into_iter().chain(m.iter().copied()).chain(s.iter().copied())) {
    ...
    debug_assert!(!residues.is_empty(), "satisfiable shot gate lost all residues");
    ...
    Outcome::Rewrite { seq, kind: RuleKind::R1, dropped }
}
...
fn r2(g: &XGate, h: &XGate, k_max: usize, rng: &mut impl Rng) -> Outcome {
    ...
    match XGate::conj(b, [(a, !pa)].into_iter().chain(f.iter().copied()).chain(r.iter().copied())) {
    ...
    debug_assert!(!pieces.is_empty(), "satisfiable colliding gate lost all pieces");
    if pieces.iter().any(|p| p.width() > k_max) {
        return Outcome::Blocked(BlockReason::WidthCap);
    }
    let mut seq: Vec<(XGate, Role)> = pieces.into_iter().map(|p| (p, Role::CollidingPiece)).collect();
    seq.push((g.clone(), Role::ShotPiece));
    Outcome::Rewrite { seq, kind: RuleKind::R2, dropped }
}
...
fn r3(g: &XGate, h: &XGate, k_max: usize, rng: &mut impl Rng) -> Outcome {
    ...
    let mut dropped = 0usize;
    let stay = match XGate::conj(a, [(b, pb)].into_iter().chain(s.iter().copied()).chain(r.iter().copied())) {
        Some(g) => Some(g),
        None => {
            dropped += 1;
            None
    ...
    Outcome::Rewrite { seq, kind: RuleKind::R3, dropped }
}
...
pub fn verify_rewrite(before: &[XGate], after: &[XGate]) -> bool {
    ...
    while v < total {
        ...
        let valid: u64 = if total - v >= 64 { !0 } else { (1u64 << (total - v)) - 1 };
        for i in 0..k {
