...
impl SwapWordEngine {
    pub fn new() -> SwapWordEngine {
        ...
        back.insert(IDENT, BackWord { len: 0, gates: [0; 4] });
        let mut frontier: Vec<(u64, BackWord)> =
            vec![(IDENT, BackWord { len: 0, gates: [0; 4] })];
        for depth in 1..=4u8 {
        ...
        engine.bare =
            engine.solve(engine.s_ab, MAX_WORD).expect("S_ab is reachable");
        assert_eq!(engine.bare.len(), 6, "dist(S_ab) must be 6 over the 24-gate alphabet");
        engine
    }
...
mod tests {
    ...
    fn word_perm(e: &SwapWordEngine, w: &[u8]) -> u64 {
        w.iter().fold(IDENT, |p, &g| compose(p, e.gate_perm(g as usize)))
    }
    ...
    fn engine_solutions_verify_and_match_census() {
        ...
        for &h1 in &on3 {
            for &h2 in &on3 {
                ...
                let Some(w) = e.solve(target, MAX_WORD) else { continue };
                assert_eq!(word_perm(&e, &w), target);
        ...
        assert_eq!(seen, 30);
        assert_eq!(len4, 12, "census: 12/30 ordered pairs at length 4 for fixed S_ab");
    }
