...
use super::engine::{Engine, Params};
use super::rules::{self, Outcome, Role};
use super::xgate::{XGate, eval_lanes};
...
fn random_circuit(n: u16, m: usize, rng: &mut impl Rng) -> Vec<XGate> {
    (0..m).map(|_| XGate::from_g57(random_g57(n, rng))).collect()
}
...
fn random_conj(target: u16, w: usize, n: u16, rng: &mut impl Rng) -> XGate {
    ...
    XGate::conj(target, wires[..w].iter().map(|&x| (x, rng.random_bool(0.5)))).unwrap()
}
...
fn cross_random_pairs() {
    ...
    assert!(seen.iter().all(|&c| c > 100), "rule coverage too thin: {seen:?}");
}
...
fn engine_small_circuit_exhaustive() {
    ...
    let mut e = Engine::new(
        gates.clone(),
        Params { k_max: 4, size_bound: 240, verify_every: 8, report_every: 1 << 30, seed: 42, ..Params::default() },
    );
    e.run();
    let out = e.arena.to_vec();
    assert!(out.len() >= 240, "size bound not reached: {}", out.len());
    assert!(eq_exhaustive(&gates, &out, n as usize), "engine broke the function");
    ...
    assert!(eq_exhaustive(&gates, &floated, n as usize), "final float broke the function");
}
