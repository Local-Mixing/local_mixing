...
impl XGate {
    ...
    /// Scalar application for exhaustive tests and small-circuit tools.
    #[inline]
    ...
    /// Scalar application on the widest circuit arithmetic used by `sss`.
    #[inline]
    pub fn apply_u1024(&self, state: U1024) -> U1024 {
        let one = U1024::one();
        let mut fires = true;
        for &(wire, positive) in &self.ctrls {
...
pub fn eval_u1024<'a>(gates: impl IntoIterator<Item = &'a XGate>, mut state: U1024) -> U1024 {
    for gate in gates {
