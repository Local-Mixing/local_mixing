//! circuit_mixer CLI options and precedence-aware resolution of DB policy layers.
use clap::Parser;

#[derive(Parser, Debug)]
#[command(
    name = "circuit_mixer",
    about = "Run database mixing, splitting, or crossing on a reversible circuit"
)]
pub struct Args {
    #[command(flatten)]
    ...
    /// other move weights 0 this makes circuit_mixer a pure match-rate sampler.
    #[arg(long, default_value_t = false)]
    ...
    /// TDP profile: the DB settings for running circuit_mixer on a gadgetized sliced
...
/// 12" from "--tdp set 12" -- so precedence was decided by mutation order and
...
impl DbLayer {
    ...
    /// Bottom layer: what circuit_mixer ships when the user says nothing at all. These
    ...
    ///
    /// Note the asymmetry with presets, which is deliberate: `--tdp` DOES keep
    ...
    /// The TDP profile: the DB block for a gadgetized-sliced-sandwich input.
    /// Deliberately silent on p_mix -- that is layer 2's lever.
    pub(super) fn tdp() -> DbLayer {
        DbLayer {
