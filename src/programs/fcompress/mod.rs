//! fcompress application adapter: circuit I/O, slice controls, verification and final artifacts.
use crate::circuit::formats as format;
use crate::circuit::formats::expand_packed;
use crate::circuit::xgate::{XGate, eval_lanes, max_wire};
use crate::engine::mixer::Mixer;
use crate::stages::post_processing::compression::{
...
pub struct Args {
    ...
    #[arg(long, default_value_t = 0)]
    seed: u64,
    /// Per-gate ancestry sidecar for the INPUT circuit (written by circuit_mixer
    ...
    if let Some(path) = &args.output {
        match &packed {
            Some(p) => {
                ...
                println!(
                    "[fcompress] wrote {} packed gates to {} (esop1)",
                    p.len(),
                    path
                );
            }
