...
fn print_sat_cone(label: &str, gates: &[[u16; 3]], cone_range: Option<(usize, usize)>) {
    ...
    if let Some(stats) =
        crate::r_ssg::sat_score::output_cone_stats(gates, output_start, output_bits)
    {
        println!(
...
pub fn main_shuffle_shoot_shuffle(
    ...
    let mut total_t = crate::r_ssg::transpositions::Transpositions {
    ...
    use crate::r_ssg::transpositions::{
    ...
    let track = crate::r_ssg::replace::track_survivors();
    ...
    let sidecar: Option<Vec<Tag>> = if track && !do_gadgetize && !do_feistalize {
        crate::r_ssg::replace::read_tags_sidecar(source)
    } else {
        None
    };
    let mut survivor_tags: Vec<Tag> = if !track {
        ...
        crate::r_ssg::replace::bump_litter_ids_past(&loaded);
        println!(
            "[tags] resumed {} gate tags from {}.tags (litter ids continue above prior max)",
            loaded.len(),
            source
        );
        crate::r_ssg::replace::gen_report("[tags] resumed state:", &loaded);
        loaded
    } else if crate::r_ssg::replace::gen_mode() {
    ...
    let samf_target = crate::r_ssg::replace::SAMF_TARGET.load(Relaxed);
    let mut m_eff = m;
    ...
    let grow_permille = crate::r_ssg::replace::GROW_THRESHOLD_PERMILLE.load(Relaxed);
    ...
    let compress_fraction_permille =
        crate::r_ssg::replace::COMPRESS_FRACTION_PERMILLE.load(Relaxed);
    ...
    // is requested.
    let target_size = crate::r_ssg::replace::TARGET_SIZE.load(Relaxed);
    ...
    loop {
        ...
        if stage_d {
            ...
            crate::r_ssg::replace::SHOOT_SIZE_CAP.store(cap, std::sync::atomic::Ordering::Relaxed);
            println!(
        ...
        crate::r_ssg::replace::REC_ROUND.store(i + 1, std::sync::atomic::Ordering::Relaxed);
        ...
        if egg {
            ...
            use crate::r_ssg::transpositions::shuffled_shoot_then_samf_core;
            ...
            use crate::r_ssg::transpositions::shuffled_shoot_then_samf;
        ...
        if single_end && i == rounds - 1 {
            use crate::r_ssg::transpositions::apply_unsamf;
            apply_unsamf(
                &mut circuit.gates,
                &total_t,
                &total_neg,
        ...
        // Record + print this round's SAMF stats (deltas from the previous round).
        {
            ...
            let unclean = crate::r_ssg::transpositions::UNCLEAN_EXPANSIONS.load(Relaxed);
            println!(
            ...
            if crate::r_ssg::replace::litter_rules() && crate::r_ssg::replace::gen_mode() {
                crate::r_ssg::replace::litter_report("    [litter] cumulative:");
            }
        ...
        }
        {
            ...
            if track {
                crate::r_ssg::replace::write_tags_sidecar(&round_path, &survivor_tags);
            }
            if equality_check {
        ...
        if stage_d {
            ...
            let permille = crate::r_ssg::replace::MIN_GEN_PERMILLE.load(Relaxed);
    ...
    crate::r_ssg::replace::SHOOT_SIZE_CAP.store(0, std::sync::atomic::Ordering::Relaxed);

    if track && crate::r_ssg::replace::gen_mode() {
    ...
    println!(
        ...
        crate::r_ssg::replace::FORCED_COLLISIONS.load(std::sync::atomic::Ordering::Relaxed)
    );
    ...
    if track {
        crate::r_ssg::replace::write_tags_sidecar(save, &survivor_tags);
    }
