...
use crate::{
    ...
    random::random_data::{
        contiguous_convex_tagged as contiguous_convex, find_convex_subcircuit_max_gates,
        find_convex_subcircuit_max_wires, simple_find_convex_subcircuit,
    },
};
use rand::Rng;
...
mod tag_tests {
    ...
    #[test]
    fn generation_litter_fields_are_independent_and_saturating() {
        let a = Tag::new_litter(17, 23);
        let b = Tag::new_litter(u32::MAX, usize::MAX);

        assert_eq!(a.generation(), 17);
        assert_eq!(a.litter_size(), 23);
        assert_eq!(b.generation(), 16_383);
        assert_eq!(b.litter_size(), 1_023);
        assert_ne!(a.litter_id(), b.litter_id());
    }
}
...
pub fn cand_features(
    ...
    left: &[[u16; 3]],
    right: &[[u16; 3]],
) -> crate::r_ssg::ranking::CandFeatures {
    use crate::circuit::circuit::Gate;
    use crate::r_ssg::ranking::CandFeatures;
    let thr = MIN_MEDIAN_LEEWAY.load(Ordering::Relaxed);
    let n = window.len();
        ...
        if sub.gates.len() == 2 {
            match pair_mode {
                ExpandPairMode::Curated => {
                    use crate::r_ssg::pairs::expand_curated_db;
...
pub fn compress_db(
    ...
    for _ in 0..trials {
        ...
        let repl: CircuitSeq = if gen_mode() {
            ...
            let feats: Vec<crate::r_ssg::ranking::CandFeatures> = min_set
                .iter()
                .map(|cc| cand_features(&cc.gates, left, right))
                .collect();
            let order = crate::r_ssg::ranking::incoming().order(&feats);
...
pub fn print_compress_timers() {
    ...
    use crate::r_ssg::transpositions::{SAMF_COMPRESSIONS_FAILED, SAMF_COMPRESSIONS_MADE};
