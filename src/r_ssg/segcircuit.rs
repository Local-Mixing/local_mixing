...
impl SamfTail {
    ...
    pub fn from_transpositions(
        t: &crate::r_ssg::transpositions::Transpositions,
        neg: &[u8],
        n: usize,
    ) -> Self {
        SamfTail {
    ...
    pub fn then(&self, other: &SamfTail) -> SamfTail {
        let n = self.perm.len();
        let perm: Vec<u16> = (0..n).map(|w| other.perm[self.perm[w] as usize]).collect();
...
pub struct SegCircuit {
    chunks: Vec<Chunk>,
    total: usize,
    // Exact global generation histogram. Stage B/C asks for the minimum,
    // fractional floor, and count-below threshold before every pass. Keeping
    // these counts current at splice time avoids rebuilding a BTreeMap from
    // every tag on each scheduler iteration.
    gen_counts: BTreeMap<u32, usize>,
}
...
impl SegCircuit {
    ...
    pub fn from_flat(gates: &[[u16; 3]], tags: &[Tag]) -> Self {
        ...
        let mut gen_counts = BTreeMap::new();
        for &tag in tags {
            *gen_counts.entry(tag.generation()).or_insert(0) += 1;
        }
        let mut i = 0;
        while i < gates.len() {
        ...
        out.debug_assert_generation_index();
        out
    }
    ...
    /// are written off. `skip = 0` reduces to the absolute minimum. O(#distinct generations).
    pub fn frac_min_gen(&self, skip: usize) -> u32 {
        ...
        let mut acc = 0usize;
        let mut last = 0u32;
        for (&generation, &count) in &self.gen_counts {
            last = generation;
            acc += count;
            if acc > skip {
                return generation;
    ...
    /// Count gates whose generation tag is < `target`.
    ...
    pub fn count_zero(&self) -> usize {
        self.gen_counts.get(&0).copied().unwrap_or(0)
    }
    ...
    pub fn splice(
        ...
        assert_eq!(
            new_gates.len(),
            new_tags.len(),
            "gates/tags length mismatch"
        );
        assert!(index + remove_len <= self.total, "splice out of range");
        // Aggregate by generation before touching the global BTreeMap. A
        // typical window has thousands of tags but only a handful of
        // generations, so this performs one tree update per distinct changed
        // generation rather than one per gate.
        let mut gen_deltas: HashMap<u32, (usize, usize)> = HashMap::new();
        if remove_len > 0 {
            let (mut ci, mut off) = self.locate(index);
            let mut need = remove_len;
            while need > 0 {
                let chunk = &self.chunks[ci];
                let take = (chunk.len() - off).min(need);
                for &tag in &chunk.tags[off..off + take] {
                    gen_deltas.entry(tag.generation()).or_default().0 += 1;
                }
                need -= take;
                ci += 1;
                off = 0;
        ...
        for &tag in new_tags {
            gen_deltas.entry(tag.generation()).or_default().1 += 1;
        }
        if self.chunks.is_empty() {
            self.chunks
                .push(Chunk::new(new_gates.to_vec(), new_tags.to_vec()));
            self.total = new_gates.len();
            self.apply_generation_deltas(gen_deltas);
            self.debug_assert_generation_index();
            return;
        }
        ...
        if start_ci == end_ci {
            ...
            c.gates
                .splice(start_off..end_off, new_gates.iter().copied());
            ...
            self.chunks
                .splice(start_ci..=end_ci, std::iter::once(merged));
        }
    ...
    fn apply_generation_deltas(&mut self, deltas: HashMap<u32, (usize, usize)>) {
        for (generation, (removed, added)) in deltas {
            let current = self.gen_counts.get(&generation).copied().unwrap_or(0);
            let next = current
                .checked_sub(removed)
                .expect("generation count underflow")
                .checked_add(added)
                .expect("generation count overflow");
            if next == 0 {
                self.gen_counts.remove(&generation);
            } else {
                self.gen_counts.insert(generation, next);
            }
        }
    }

    #[inline]
    fn debug_assert_generation_index(&self) {
        debug_assert!(self.gen_counts.values().all(|&count| count > 0));
        debug_assert_eq!(self.gen_counts.values().sum::<usize>(), self.total);
        let chunk_min = self
            .chunks
            .iter()
            .map(|chunk| chunk.min_gen)
            .min()
            .unwrap_or(u32::MAX);
        debug_assert_eq!(self.min_gen(), chunk_min);
    }
...
mod tests {
    ...
    fn reference_frac_min(tags: &[Tag], skip: usize) -> u32 {
        if tags.is_empty() {
            return 0;
        }
        let mut counts = BTreeMap::new();
        for &tag in tags {
            *counts.entry(tag.generation()).or_insert(0usize) += 1;
        }
        let mut seen = 0usize;
        let mut last = 0u32;
        for (generation, count) in counts {
            last = generation;
            seen += count;
            if seen > skip {
                return generation;
    ...
    fn reference_random_frac_min_index(
        tags: &[Tag],
        skip: usize,
        rng: &mut impl Rng,
    ) -> Option<usize> {
        ...
        let generation = reference_frac_min(tags, skip);
        let mut chosen = None;
        let mut seen = 0usize;
    ...
    fn assert_generation_index(sc: &SegCircuit, tags: &[Tag]) {
        let mut expected = BTreeMap::new();
        for &tag in tags {
            *expected.entry(tag.generation()).or_insert(0usize) += 1;
        }
        assert_eq!(sc.gen_counts, expected);
        assert_eq!(sc.gen_counts.values().sum::<usize>(), tags.len());
        assert!(sc.gen_counts.values().all(|&count| count > 0));
        assert_eq!(
            sc.min_gen(),
            tags.iter()
        ...
        for target in [0u32, 1, 2, 5, 17, 1024, 16_383, u32::MAX] {
            assert_eq!(
                sc.count_below(target),
                tags.iter().filter(|tag| tag.generation() < target).count()
            );
        }
        assert_eq!(
            sc.count_zero(),
            tags.iter().filter(|tag| tag.generation() == 0).count()
        );
        ...
        let skips = [
            ...
            tags.len(),
            tags.len().saturating_add(7),
        ];
        for skip in skips {
            let generation = reference_frac_min(tags, skip);
            assert_eq!(sc.frac_min_gen(skip), generation);
            let expected_indices: Vec<usize> = tags
                .iter()
                .enumerate()
                .filter_map(|(index, tag)| (tag.generation() == generation).then_some(index))
                .collect();
            assert_eq!(
                sc.frac_min_gen_indices(skip),
                (generation, expected_indices)
            );

            // The cached counts must not perturb the reservoir sampler or its
            // random stream: same anchor and same next draw as the flat scan.
            let mut actual_rng = StdRng::seed_from_u64(0x5eed ^ skip as u64);
            let mut reference_rng = StdRng::seed_from_u64(0x5eed ^ skip as u64);
            assert_eq!(
                sc.random_frac_min_gen_index(skip, &mut actual_rng),
                reference_random_frac_min_index(tags, skip, &mut reference_rng)
            );
    ...
    use crate::r_ssg::transpositions::Transpositions;
