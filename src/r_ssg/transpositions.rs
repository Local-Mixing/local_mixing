...
// would pay off: serial `core` = shuffled_shooting_game_core (the collision game + DB lookups per
// window), `reconcile` = per-pass apply_unsamf, `merge` = SegCircuit splice + ledger index
// shifts; parallel batches report combined compute wall time plus their serial merge; `flush` =
// periodic full-circuit apply_ledger. Off (unset) = only predictable profile-flag branches.
...
// Parallel batches overlap each window's core and reconciliation work, so
// summing per-worker timings would overstate wall time. Measure the whole
// parallel compute and serial merge phases once per batch instead.
pub static SHOOT_PAR_COMPUTE_NS: std::sync::atomic::AtomicU64 =
    std::sync::atomic::AtomicU64::new(0);
pub static SHOOT_PAR_MERGE_NS: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(0);
...
fn integrate_samf_compressed(
    ...
    use_curated: bool,
    use_regular: bool,
    tags: &mut Vec<crate::r_ssg::replace::Tag>,
) {
    use crate::r_ssg::pairs::{compress_curated_db, find_any_replacement_db};
    ...
    if !use_curated && !use_regular {
        ...
        if track {
            ...
            let nt = crate::r_ssg::replace::new_gate_tag(
                &tags[tags.len() - k..],
                gates.len() - tags.len(),
            );
    ...
    if track {
        ...
        let nt = crate::r_ssg::replace::new_gate_tag(&tags[ctx_start..], gates.len() - ctx_start);
...
fn insert_m_samfs_core(
    ...
    m: usize,
    x: usize,
    input_tags: &[crate::r_ssg::replace::Tag],
) -> (
    Vec<[u16; 3]>,
    Transpositions,
    Vec<u8>,
    Vec<crate::r_ssg::replace::Tag>,
) {
    ...
    let mut gates: Vec<[u16; 3]> = Vec::new();
    let mut out_tags: Vec<crate::r_ssg::replace::Tag> = Vec::new();
    let mut negation_mask = vec![0u8; n];
    for (i, gate) in input.iter().enumerate() {
        ...
        if track {
            ...
            out_tags.resize(
                gates.len() - 1,
                crate::r_ssg::replace::new_gate_tag(
...
pub fn insert_wire_m_samfs_every_x(
    ...
    println!("Starting len: {} gates", circuit.gates.len());
    let (mut gates, t_list, negation_mask, _itags) =
...
fn flush_relabelled_gate_controls(
    ...
    flush_gate_controls(relabel_gate(gate, t_list), output, negation_mask, n);
}

#[inline]
fn flush_gate_controls(
    relabelled: [u16; 3],
    output: &mut Vec<[u16; 3]>,
    negation_mask: &mut [u8],
    n: usize,
...
fn emit_relabelled_gate(
    ...
    let relabelled = relabel_gate(gate, t_list);
    flush_gate_controls(relabelled, output, negation_mask, n);
    output.push(relabelled);
}
...
fn shuffled_shooting_game_core(
    ...
    use crate::r_ssg::pairs::{compress_curated_db, expand_curated_db_neg};
    ...
    while materialized_shot.is_some() || !remaining.is_empty() {
        ...
        if !shot_is_materialized {
            ...
            if track {
                ...
                output_tags.resize(
                    output.len(),
                    crate::r_ssg::replace::new_gate_tag(std::slice::from_ref(&shot_tag), new_count),
                );
            }
        }
        ...
        let forced_collision =
            !has_collision && crate::r_ssg::replace::gen_mode() && !passed.is_empty();
        if forced_collision {
            ...
            has_collision = true;
            crate::r_ssg::replace::FORCED_COLLISIONS
        ...
        let replaced = 'try_replace: {
            ...
            let (consumed_indices, window, expansion, picked_negated, picked_full_litter): (
                ...
                if crate::r_ssg::replace::gen_mode() {
                ...
                let order: Vec<usize> = if crate::r_ssg::replace::gen_mode() {
                    let left = &output[output.len().saturating_sub(FEATURE_CTX)..];
                    let feats: Vec<crate::r_ssg::ranking::CandFeatures> = candidates
                        .iter()
                        .map(|(idxs, win)| {
                            ...
                            let right: Vec<[u16; 3]> = (0..rg.len())
                                ...
                                .map(|m| rg[m])
                                .collect();
                            crate::r_ssg::replace::cand_features(win, left, &right)
                        })
                        .collect();
                    ...
                    let mut ord = crate::r_ssg::ranking::outgoing().order(&feats);
                ...
                let litter_active = track
                    && crate::r_ssg::replace::gen_mode()
                    && crate::r_ssg::replace::litter_rules();
                ...
                let order: Vec<usize> = if litter_active {
                    litter_stats = candidates
                        ...
                        .map(|(idxs, _)| {
                            ...
                            crate::r_ssg::replace::window_litter_stats(&wt)
                        })
                        .collect();
                    let mut ord = order;
                    ord.sort_by_key(|&i| {
                        std::cmp::Reverse(if litter_stats[i].1 {
                            0
                        } else {
                            litter_stats[i].0
                        })
                ...
                let gen_m = crate::r_ssg::replace::gen_mode();
                        ...
                        expand_curated_db_neg(win, n, db, use_curated, use_regular, &neg)
                    {
                        if e.len() >= 3 {
                            ...
                            if litter_active && !full {
                                ...
                                if distinct >= 2 {
                                    crate::r_ssg::replace::LITTER_TIER1
                                        .fetch_add(1, Ordering::Relaxed);
                                    crate::r_ssg::replace::LITTER_DISTINCT_SUM
                                        .fetch_add(distinct, Ordering::Relaxed);
                                } else {
                                    crate::r_ssg::replace::LITTER_TIER2
                                        .fetch_add(1, Ordering::Relaxed);
                                }
                            }
            ...
            if picked_negated.is_empty() && crate::r_ssg::replace::verify_db_hits() {
                let win_c = crate::circuit::circuit::CircuitSeq {
                    gates: window.clone(),
                };
                let exp_c = crate::circuit::circuit::CircuitSeq {
                    gates: expansion.clone(),
                };
            ...
            let event_gen: u32 = if track && crate::r_ssg::replace::gen_mode() {
                let mut win: Vec<Tag> = Vec::with_capacity(consumed);
                win.push(shot_tag);
                win.extend(consumed_indices.iter().map(|&idx| remaining_tags[idx]));
                crate::r_ssg::replace::median_floor_gen(&win).saturating_add(1)
            } else {
                0
            };
            ...
            let cone_aware = crate::r_ssg::sat_score::sat_cone_aware_enabled();
            let pair_attempts = if cone_aware {
                crate::r_ssg::sat_score::sat_hidden_samf_candidates()
            } else {
                samf_hide_pairs()
            };
            'pairs: for pair_idx in 0..pair_attempts {
                ...
                for &neg_type in candidate_types.choose_multiple(&mut rng, type_attempts.max(1)) {
                    ...
                    if let Some(repl) = compress_curated_db(&window, n, db, false, use_regular) {
                        ...
                        if samf_hidden {
                            let score = if cone_aware {
                                let sat_score = crate::r_ssg::sat_score::score_subcircuit(
                                    &repl,
                                    n,
                                    crate::r_ssg::sat_score::sat_score_seed()
                                        ^ ((pair_idx as u64) << 8)
                                        ^ neg_type as u64,
                                );
                                crate::r_ssg::sat_score::compression_selection_score(&sat_score)
                            } else {
                                0.0
                            };
            ...
            if picked_full_litter && tuck.is_none() && picked_negated.is_empty() {
                crate::r_ssg::replace::LITTER_ABORT_NO_SAMF.fetch_add(1, Ordering::Relaxed);
                break 'try_replace false;
            }
            if picked_full_litter && tuck.is_some() {
                crate::r_ssg::replace::LITTER_SAMF_LICENSED.fetch_add(1, Ordering::Relaxed);
            }
            ...
            crate::r_ssg::replace::record_expansion(
                consumed,
                expansion.len(),
                before_wires,
            ...
            let event_tag = if track && crate::r_ssg::replace::gen_mode() {
            ...
            if crate::r_ssg::replace::record_enabled() {
                let rec_end = output.len();
                let rs = rec_start.min(rec_end);
                let ws: Vec<u16> = output[rs..rec_end].iter().flatten().copied().collect();
                crate::r_ssg::replace::record_replacement(
                    "expand",
                    crate::r_ssg::replace::REC_PASS.load(std::sync::atomic::Ordering::Relaxed),
                    rs,
                    rec_end,
                    consumed,
                    &ws,
    ...
    while let Some(g) = remaining.pop_front() {
        ...
        if track {
            ...
            output_tags.resize(
                output.len() - 1,
                crate::r_ssg::replace::new_gate_tag(std::slice::from_ref(&gt), new_count),
            );
            output_tags.push(gt);
        }
    }
...
fn shuffled_shooting_game_repeated_core(
    ...
    for pass in 0..passes {
        crate::r_ssg::replace::REC_PASS.store(pass + 1, std::sync::atomic::Ordering::Relaxed);
...
pub fn shuffled_shoot_then_samf_core(
    ...
    if crate::r_ssg::replace::gen_mode() && track {
        use crate::r_ssg::segcircuit::{SamfLedger, SamfTail, SegCircuit};
        use std::sync::atomic::Ordering::Relaxed;
        const WINDOW_CAP: usize = 8192; // max gates a single short pass spans
        let mut rng = rand::rng();
        let min_gen_target = crate::r_ssg::replace::MIN_GEN.load(Relaxed) as u32;
        let pass_length = crate::r_ssg::replace::PASS_LENGTH.load(Relaxed);
        let max_passes = crate::r_ssg::replace::MAX_PASSES.load(Relaxed);
        let min_gen_permille = crate::r_ssg::replace::MIN_GEN_PERMILLE.load(Relaxed);
        // Stage D: pause shooting (return to the driver to compress) once the working circuit
        // reaches this many gates. 0 = no cap.
        let shoot_size_cap = crate::r_ssg::replace::SHOOT_SIZE_CAP.load(Relaxed);
        ...
        if centralize_pct > 0 {
            println!(
                "[gen] CENTRALIZE active: central {}% band, gen=min+1, bidirectional",
                centralize_pct
            );
        }
        ...
        let global_check = |sc: &SegCircuit,
                            ledger: &SamfLedger,
                            acc_net: &SamfTail,
                            where_: &str|
         -> bool {
            ...
            let all: Vec<(usize, SamfTail)> = ledger
                .entries()
                .iter()
                .map(|(p, t)| (*p, t.clone()))
                .collect();
        ...
        let do_right_pass =
            |sc: &mut SegCircuit, ledger: &mut SamfLedger, anchor: usize| -> usize {
                if anchor >= sc.len() {
                    return 0;
                }
                ...
                if profile {
                    ...
                    SHOOT_CORE_NS.fetch_add(
                        t1.unwrap().duration_since(t0.unwrap()).as_nanos() as u64,
                        Relaxed,
                    );
                    SHOOT_RECONCILE_NS.fetch_add(
                        t2.unwrap().duration_since(t1.unwrap()).as_nanos() as u64,
                        Relaxed,
                    );
                    SHOOT_MERGE_NS
                        .fetch_add(t3.duration_since(t2.unwrap()).as_nanos() as u64, Relaxed);
            ...
            |sc: &mut SegCircuit, ledger: &mut SamfLedger, gate_idx: usize| -> (usize, usize) {
                ...
                if stagec_check {
                    ...
                    if !stagec_pass_ok(&out, &wg_orig, &identity, n) {
                        eprintln!(
                            "[LEFT-PASS BREAK] lo={lo} wlen={wlen} out_len={}",
                            out.len()
                        );
                        std::process::exit(8);
                    }
        ...
        loop {
            ...
            if ledger.len() >= LEDGER_FLUSH_AT {
                ...
                if stagec_check
                    && !global_check(
                        ...
                        &ledger,
                        &acc_net,
                        &format!("PRE-flush pass {pass_idx}"),
                    )
                {
                    std::process::exit(9);
                }
                ...
                if stagec_check
                    && !global_check(
                        ...
                        &ledger,
                        &acc_net,
                        &format!("POST-flush pass {pass_idx}"),
                    )
                {
                    std::process::exit(9);
                }
            }
            crate::r_ssg::replace::REC_PASS.store(pass_idx + 1, Relaxed);
            ...
            if let Some(g) = prioritized {
                ...
                let mut results: Vec<(usize, usize, Vec<[u16; 3]>, Vec<Tag>, SamfTail, usize)> =
                    anchors
                        ...
                        .map(|&anchor| {
                            ...
                            let (mut out, t_p, neg_p, comp, mut out_t) =
                                shuffled_shooting_game_core(
                                    &wg,
                                    n,
                                    db,
                                    use_curated,
                        ...
                        .collect();
                if let Some(t0) = par_compute_t0 {
                    SHOOT_PAR_COMPUTE_NS.fetch_add(t0.elapsed().as_nanos() as u64, Relaxed);
                }
                let (profiled_passes, profiled_wlen) = if profile {
                    (
                        results.len(),
                        results.iter().map(|result| result.1).sum::<usize>(),
                    )
                } else {
                    (0, 0)
                };
                let par_merge_t0 = if profile {
                    Some(std::time::Instant::now())
                } else {
                    None
                };
                ...
                if let Some(t0) = par_merge_t0 {
                    SHOOT_PAR_MERGE_NS.fetch_add(t0.elapsed().as_nanos() as u64, Relaxed);
                    SHOOT_RIGHT_PASSES.fetch_add(profiled_passes, Relaxed);
                    SHOOT_WLEN_SUM.fetch_add(profiled_wlen, Relaxed);
                }
            } else {
        ...
        if stagec_check {
            ...
            if fin.probably_equal(&inp, n, 3000).is_err() {
                eprintln!(
                    "[STAGEC OUTPUT BREAK] Stage C output != input (bug is in Stage C), pass_idx={pass_idx}"
                );
            } else {
                eprintln!(
                    "[STAGEC OUTPUT OK] Stage C output == input; any later failure is in COMPRESSION, pass_idx={pass_idx}"
                );
            }
        }
        ...
        if profile {
            ...
            let par_compute = SHOOT_PAR_COMPUTE_NS.swap(0, Relaxed);
            let par_merge = SHOOT_PAR_MERGE_NS.swap(0, Relaxed);
            let rp = SHOOT_RIGHT_PASSES.swap(0, Relaxed);
            let lp = SHOOT_LEFT_PASSES.swap(0, Relaxed);
            let wl = SHOOT_WLEN_SUM.swap(0, Relaxed);
            let tot = (core + rec + mrg + flu + par_compute + par_merge).max(1) as f64;
            let pct = |x: u64| x as f64 * 100.0 / tot;
            println!(
                "[shoot-profile] serial-core {:.1}s ({:.0}%) | serial-reconcile {:.1}s ({:.0}%) | serial-merge {:.1}s ({:.0}%) | parallel-compute {:.1}s ({:.0}%) | parallel-merge {:.1}s ({:.0}%) | flush {:.1}s ({:.0}%) || right {} left {} avg_wlen {}",
                core as f64 / 1e9,
                pct(core),
                rec as f64 / 1e9,
                pct(rec),
                mrg as f64 / 1e9,
                pct(mrg),
                par_compute as f64 / 1e9,
                pct(par_compute),
                par_merge as f64 / 1e9,
                pct(par_merge),
                flu as f64 / 1e9,
                pct(flu),
                rp,
        ...
        if track {
            ...
            if (pass + 1) % every == 0 || pass + 1 == passes {
                if crate::r_ssg::replace::gen_mode() {
                    ...
                    let alive = tags
                        .iter()
                        .filter(|&&t| t != crate::r_ssg::replace::Tag::NEW)
                        .count();
                    println!(
...
fn apply_ledger(
    gates: &[[u16; 3]],
    tags: &[Tag],
    entries: &[(usize, crate::r_ssg::segcircuit::SamfTail)],
    n: usize,
) -> (Vec<[u16; 3]>, Vec<Tag>, crate::r_ssg::segcircuit::SamfTail) {
    use crate::r_ssg::segcircuit::SamfTail;
    let track = !tags.is_empty();
    let mut t_list = Transpositions {
        transpositions: Vec::new(),
    };
    ...
    for (i, &g) in gates.iter().enumerate() {
        ...
        if track {
            ...
            if added > 1 {
                let nt = crate::r_ssg::replace::new_gate_tag(&[tags[i]], added - 1);
...
mod reversed_samf_tests {
    use super::{
        SWAP_N1_3W, SWAP_N1_4W, SWAP_N2_3W, SWAP_N2_4W, Transpositions, emit_relabelled_gate,
        neg_flips, random_neg_type, shoot_gate_to_first_collision,
    ...
    #[test]
    fn evaluate_matches_ordered_borrowed_swaps() {
        let t = Transpositions {
            transpositions: vec![(0, 4, 0), (2, 4, 3), (1, 5, 1), (0, 5, 2)],
        };
        for input in 0u16..8 {
            ...
            assert_eq!(t.evaluate(input), expected);
        }
    }

    #[test]
    fn emit_relabels_once_into_current_wire_space() {
        let t = Transpositions {
            transpositions: vec![(0, 4, 0), (2, 5, 0)],
        };
        let mut output = Vec::new();
        let mut negation_mask = [0u8; 8];

        emit_relabelled_gate([0, 1, 2], &mut output, &t, &mut negation_mask, 8);

        assert_eq!(output, vec![[4, 1, 5]]);
        assert_eq!(negation_mask, [0u8; 8]);
    }
