//! Frozen-table: static, compressed, point-lookup store for the circuit DB.
//!
...
//!
//! Subcommands:
//!   frozen_table tables   <rocks> <out_dir> [per_range]
//!   frozen_table write    <rocks> <out_dir>
//!   frozen_table validate <rocks> <out_dir>
...
// ---------------------------------------------------------------- stages
fn stage_tables(rocks: &str, out_dir: &str, per_range: usize) {
    let db = DB::open_for_read_only(&Options::default(), rocks, false).expect("open rocks");
...
fn stage_write(rocks: &str, out_dir: &str) {
...
fn main() {
    ...
    match args.get(1).map(|s| s.as_str()) {
        Some("tables") => {
            let per: usize = args.get(4).and_then(|s| s.parse().ok()).unwrap_or(400_000);
            stage_tables(&args[2], &args[3], per);
        }
        Some("write") => stage_write(&args[2], &args[3]),
        Some("validate") => stage_validate(&args[2], &args[3]),
        _ => {
            eprintln!("usage: frozen_table tables|write|validate <rocks> <dir> [per_range]");
            std::process::exit(2);
        }
    }
}
