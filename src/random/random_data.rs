...
use crate::circuit::circuit::{
    canonicalize_polys, canonicalize_polys_2, canonicalize_polys_3, canonicalize_polys_4,
};
...
mod tests {
    ...
    fn test_canonicalize_hardcoded() {
        ...
        println!("\n=== Rewired Circuit ===");
        for (i, gate) in rewired.gates.iter().enumerate() {
            println!("  gate {}: {:?}", i, gate);
        }

        println!("\n=== Permutation ===");
        println!("  final order: {:?}", final_order.data);
        // derive from comparing original vs rewired gate indices if not directly returned
        println!("  used reverse: {}", reversed);
    }
