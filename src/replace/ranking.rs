...
impl ScriptRanker {
    ...
    fn to_map(f: &CandFeatures) -> rhai::Map {
        ...
        m.insert("low_leeway_count".into(), (f.low_leeway_count as i64).into());
        m.insert("zero_fanout_count".into(), (f.zero_fanout_count as i64).into());
...
impl Ranker for ScriptRanker {
    fn order(&self, c: &[CandFeatures]) -> Vec<usize> {
        let arr: rhai::Array = c.iter().map(|f| rhai::Dynamic::from(Self::to_map(f))).collect();
        let mut scope = rhai::Scope::new();
        let res: Result<rhai::Array, _> =
            self.engine
                .call_fn(&mut scope, &self.ast, "rank", (arr,));
        match res {
            Ok(order) => {
