...
use crate::engine::mixer::{Meta, Mixer, Tap};
...
impl Mixer {
    ...
    pub(crate) fn split_twist_move(&mut self) {
        ...
        match picked {
            ...
            Some((h_id, h_comp, crossed)) => {
                ...
                // Span as a fraction of the circuit: in a piecewise round
                // the driver passes the whole circuit's size as span_norm so
                // the histogram stays frac-of-circuit, not frac-of-piece.
                let norm = if self.params.span_norm > 0 {
                    self.params.span_norm
                } else {
                    self.arena.len()
                };
                let frac20 = (s * 20) / norm.max(1);
    ...
    fn pick_bracket(&mut self, w: u16, g1: u32, g_rank: u32, d: Dir) -> Option<(u32, bool, bool)> {
        ...
        // Midpoint-crossing gauge. In a piecewise round the piece sits at
        // rank_base inside a rank_total-gate circuit, so the midpoint is the
        // whole circuit's, not the piece's (statistics only).
        let (base, total) = if self.params.rank_total > 0 {
            (self.params.rank_base, self.params.rank_total)
        } else {
            (0, self.rank_n as u32)
        };
        let mid = total / 2;
        let crossed =
            (base.saturating_add(g_rank) < mid) != (base.saturating_add(self.rank_of(id)) < mid);
    ...
    pub(crate) fn restamp_ranks(&mut self) {
        self.rank.clear();
    ...
    pub(crate) fn plant_taps(&mut self) {
    ...
    fn end_split_stage(&mut self, reason: &'static str) {
        self.split_on = false;
        self.split_ended = true;
        self.split_done = true;
        self.split_end_reason = Some(reason);
        // A piece mixer stays silent: the piecewise driver prints one stage
        // summary from the whole-circuit mixer's folded counters.
        if self.quiet {
            return;
        }
        self.announce_split_end(reason);
    }

    /// The stage-boundary report: the ENDED line, the span histogram and the
    /// canary deciles. Separate from `end_split_stage` so a piecewise run can
    /// print it once, from the whole-circuit mixer.
    ...
    pub fn split_tap_summary(&mut self) {
        if self.taps.is_empty() || self.taps_reported || self.quiet {
            return;
        }
