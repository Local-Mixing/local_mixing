//! Quadratic masking computation (formerly Ran balanced / blinded V5). Takes a circuit `A` on `n` wires and builds an equivalent circuit
...
/// Configuration for [`preprocess_quadratic_masking`].
#[derive(Clone, Copy, Debug)]
pub struct QuadraticMaskingParams {
...
impl QuadraticMaskingParams {
    /// Effective managed GSS recipe: seed from the honest source prefix while
    /// retaining the established data-and-band burst refresh policy. Dirty fire
    /// helpers use band wires independently (`QuadraticMaskingExecution`).
    pub fn managed_gss(seed: u64, active_wires: usize) -> Self {
        Self {
            active_wires,
...
#[derive(Clone, Copy, Debug)]
pub struct QuadraticMaskingExecution {
    pub ancilla_band_only: bool,
    pub diagnostics: bool,
}

impl Default for QuadraticMaskingExecution {
    fn default() -> Self {
        Self {
...
/// Result of [`preprocess_quadratic_masking`].
pub struct QuadraticMaskingOutput {
...
pub fn preprocess_quadratic_masking(
    src: &[XGate],
    np: usize,
    p: &QuadraticMaskingParams,
) -> QuadraticMaskingOutput {
    preprocess_quadratic_masking_with_execution(src, np, p, &QuadraticMaskingExecution::default())
}

/// Explicit comparison controls for retained linear-read and ancilla-pool studies.
pub fn preprocess_quadratic_masking_with_execution(
    src: &[XGate],
    np: usize,
    p: &QuadraticMaskingParams,
