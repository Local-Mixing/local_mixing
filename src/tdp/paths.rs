...
pub(crate) fn validate_external_paths(config: &ResolvedConfig) -> Result<(), String> {
    ...
    let existing_gadget = fs::metadata(config.run_dir.join("tdp.mpmct1"))
        .is_ok_and(|metadata| metadata.is_file() && metadata.len() > 0);
...
pub(crate) fn validate_run_destination(repo_root: &Path, run_dir: &Path) -> Result<(), String> {
    ...
    if run_dir.exists() {
        ...
        let has_only_wrapper_manifest =
            entries.len() == 1 && entries[0].file_name() == OsStr::new("tdp_command.conf");
...
/// Renamed artifacts must never cause an old pipeline run to look like a fresh
/// TDP run. In particular, reject it before building or creating a manifest.
pub(crate) fn reject_legacy_run(run_dir: &Path) -> Result<(), String> {
    for marker in ["gss_command.conf", "gss.mpmct1"] {
        if run_dir.join(marker).exists() {
            return Err(format!(
                "existing run_dir {} contains legacy GSS artifact {marker}; tdp_gen requires a fresh run.directory or a TDP v8 run; resume this run with the checkout that created it; the original run is unchanged",
                run_dir.display()
            ));
        }
    }
...
pub(crate) fn fresh_default_run_dir(repo_root: &Path, n: usize, tag: &str) -> PathBuf {
    let base = repo_root.join("runs").join(format!("tdp_gen_n{n}_{tag}"));
    if !base.exists() {
        return base;
    }
    for suffix in 1u32.. {
        let candidate = repo_root
            .join("runs")
            .join(format!("tdp_gen_n{n}_{tag}_{suffix:02}"));
        if !candidate.exists() {
            return candidate;
        }
    }
...
pub(crate) fn rustc_host_triple() -> Result<String, TdpError> {
    let rustc = std::env::var_os("RUSTC").unwrap_or_else(|| OsString::from("rustc"));
    let output = ProcessCommand::new(rustc)
        .arg("-vV")
        .output()
        .map_err(|error| TdpError::io(format!("cannot query the Rust host target: {error}")))?;
    if !output.status.success() {
        return Err(TdpError::child("rustc -vV", output.status));
    }
    let stdout = String::from_utf8(output.stdout)
        .map_err(|_| TdpError::io("rustc -vV returned non-UTF-8 output"))?;
    let host = stdout
        .lines()
        .find_map(|line| line.strip_prefix("host: "))
        .ok_or_else(|| TdpError::io("rustc -vV did not report a host target"))?;
    if host.is_empty()
        || !host
            .chars()
            .all(|ch| ch.is_ascii_alphanumeric() || ch == '-' || ch == '_')
    {
        return Err(TdpError::io(format!(
...
pub(crate) fn find_repo_root() -> Result<PathBuf, TdpError> {
    ...
    // scripts/tdp_gen.sh. Prefer the checkout this binary was compiled from,
    ...
    Err(TdpError::io(
        "cannot locate the repository (need Cargo.toml, configs/tdp.toml, and scripts/tdp_gen.sh)",
    ))
}

pub(crate) fn is_repo_root(path: &Path) -> bool {
    path.join("Cargo.toml").is_file()
        && path.join("configs/tdp.toml").is_file()
        && path.join("scripts/tdp_gen.sh").is_file()
}
