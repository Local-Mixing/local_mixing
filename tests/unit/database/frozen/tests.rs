...
// Swapped control convention: bytes 1 and 2 of every 3-byte gate swap, per
...
#[test]
fn qc_huffman_value_matches_unbounded_decode_for_chained_escape_forms() {
    ...
    assert_eq!(unbounded, [3, 5, 7, 11, 3, 200, 201, 202]);
    assert_eq!(
...
#[test]
fn unbounded_raw_decode_returns_all_curated_candidates() {
    let tables = Tables {
        header: rebuild_canonical(vec![(ESC, 1)]),
        gates: Vec::new(),
    };
