...
#[test]
fn malformed_length_prefixed_value_is_not_partially_accepted() {
    let err = decode_legacy_value(&[3, 0, 1]).unwrap_err();
    assert!(matches!(err, CuratedError::MalformedValue { .. }));
}

#[test]
fn checked_blob_rejects_each_byte_format_overflow() {
...
#[test]
fn composite_records_preserve_three_hundred_candidates() {
