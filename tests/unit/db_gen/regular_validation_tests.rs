...
//!
//! The canonical-key ABI guard is active and needs no artifact. The other
//! three tests need real RocksDBs and are ignored by default. Run each ignored
//! test by name with `REGULAR_DB_DIR`, or with `MERGE_PAR_DB`, `MERGE_SOURCES`,
//! and `MERGE_TEST_PREFIX` for the merge validator. Replace `TEST_NAME` below
//! with the selected ignored test's function name:
//! `cargo test --locked --features db-tools --lib db_generation::regular::validation_tests::TEST_NAME -- --ignored --exact`.
//! This file remains a private child module of
//! `db_generation::regular`, included through its `#[path]` attribute.

use std::collections::{BTreeMap, BTreeSet, HashMap};

use super::{
    abstract_gates_for_circuit_filtered, apply_wire_mapping, canonicalize_bidirectional,
...
#[test]
fn bidirectional_adapter_matches_runtime_directional_keys() {
    for variable in ["CANON_MONOMIAL_CAP", "CANON_RULE_L_BRANCH_CAP"] {
        assert!(
            std::env::var_os(variable).is_none(),
            "{variable} must be unset for the regular-DB canonical-key ABI test"
        );
    }

    fn normalized(polys: &[Polynomial]) -> Vec<Vec<u64>> {
        polys
            .iter()
            .map(|poly| {
    ...
    let circuits = [
        ...
        CircuitSeq {
            gates: vec![[4, 1, 7], [1, 7, 4], [7, 4, 1]],
        },
        CircuitSeq {
            gates: vec![[2, 0, 1], [3, 2, 0], [0, 3, 1], [1, 2, 3]],
        },
    ];

    for circuit in circuits {
        let selected = canonicalize_bidirectional(&circuit, true)
            .expect("representative circuit must canonicalize");
        let (forward, _, _) = circuit.canonicalize_polys_single(false);
        let (reverse, _, _) = circuit.canonicalize_polys_single(true);
        assert!(!forward.is_empty() && !reverse.is_empty());

        let expected = if normalized(&forward) <= normalized(&reverse) {
            &forward
        } else {
            &reverse
        };
        let selected_key = xxh3_128(&polys_repr_blob(&selected.0)).to_le_bytes();
        let expected_key = xxh3_128(&polys_repr_blob(expected)).to_le_bytes();
        assert_eq!(selected_key, expected_key);

        let decoded = CircuitSeq::from_blob(&circuit_blob(&selected.1));
        assert_eq!(decoded, selected.1, "stored circuit bytes must round-trip");
        let (stored_polys, _, _) = decoded.canonicalize_polys_single(false);
        assert_eq!(
            xxh3_128(&polys_repr_blob(&stored_polys)).to_le_bytes(),
            selected_key,
            "stored circuit must re-canonicalize to its database key"
        );
    }
}

// ============================================================ dedup validation
// For real rows from rocks_db_m7, deduplicated fresh-wire expansion must
// produce the same (key, value) sets as full concrete expansion through the
// build_from_rocks worker pipeline. Emitted + skipped must account for every
// ordered gate triple.
...
/// Reference enumeration expands every wire class into concrete candidates
/// without deduplicating fresh-wire choices.
fn reference_filtered(
    ...
    let source_used = touched.len();
    let mut result = Vec::new();
    let mut skipped = 0usize;

    let allowed = |new_wires: usize| {
        let used = source_used + new_wires;
...
fn candidate_set(circuit: &CircuitSeq, gates: &[[u16; 3]]) -> BTreeSet<(Vec<u8>, Vec<u8>)> {
    ...
    for g in gates {
        ...
        c1.canonicalize();
        if !c1.adjacent_id() {
            if let Some(canon) = canonicalize_bidirectional(&c1, true) {
                let hash: u128 = xxh3_128(&polys_repr_blob(&canon.0));
                set.insert((
                    hash.to_le_bytes().to_vec(),
                    encode_circuit(&circuit_blob(&canon.1)),
                ));
            }
        }
        // prepend
        ...
        c2.canonicalize();
        if !c2.adjacent_id() {
            if let Some(canon) = canonicalize_bidirectional(&c2, true) {
                let hash: u128 = xxh3_128(&polys_repr_blob(&canon.0));
                set.insert((
                    hash.to_le_bytes().to_vec(),
                    encode_circuit(&circuit_blob(&canon.1)),
...
fn check_dedup_circuit(circuit: &CircuitSeq, min_n: usize, label: &str) -> bool {
    ...
    let (reference_gates, reference_skip) = reference_filtered(circuit, DEDUP_N, min_n, 0);
    let (deduplicated_gates, deduplicated_skip) =
        abstract_gates_for_circuit_filtered(circuit, DEDUP_N, min_n, 0);

    let reference_ok = reference_gates.len() + reference_skip == total;
    let deduplicated_ok = deduplicated_gates.len() + deduplicated_skip == total;

    let reference_set = candidate_set(circuit, &reference_gates);
    let deduplicated_set = candidate_set(circuit, &deduplicated_gates);
    let sets_equal = reference_set == deduplicated_set;

    let pass = reference_ok && deduplicated_ok && sets_equal;
    println!(
        "[{}] u={} min_n={} reference_emitted={} deduplicated_emitted={} reference_skip={} deduplicated_skip={} reference_acct={} deduplicated_acct={} unique_kv_reference={} unique_kv_deduplicated={} sets_equal={} => {}",
        label,
        u,
        min_n,
        reference_gates.len(),
        deduplicated_gates.len(),
        reference_skip,
        deduplicated_skip,
        reference_ok,
        deduplicated_ok,
...
#[test]
#[ignore = "requires REGULAR_DB_DIR containing rocks_db_m7"]
fn fresh_wire_dedup_kv_sets_match() {
    ...
    'scan: for item in iter {
        ...
        rows_scanned += 1;
        for c in decode_rocks_entry(&key, &value).expect("valid regular DB row") {
    ...
    let mut handle = |mapping: &[u16], k_opt: Option<usize>| {
        ...
        if full_kv {
            let (canon_polys, canon_circuit, _, _, _) =
                canonicalize_bidirectional(&combined, true).unwrap();
...
#[test]
#[ignore = "requires REGULAR_DB_DIR containing rocks_db_m2 and rocks_db_m3"]
fn capped_2rocks_enumeration_matches_full() {
    ...
    let load = |db: &rocksdb::DB, limit: usize| -> Vec<CircuitSeq> {
        ...
        for item in db.iterator(rocksdb::IteratorMode::Start) {
            ...
            for circuit in decode_rocks_entry(&key, &value).expect("valid regular DB row") {
                out.push(circuit);
                if out.len() >= limit {
                    return out;
                }
    ...
    let failures: usize = jobs
        ...
        .map(|&(i, j, min_n, full_kv)| {
            ...
            for (ci, &(first, second, map_first, nf, ns)) in cases.iter().enumerate() {
                let full =
                    run_2rocks_case(first, second, map_first, nf, ns, min_n, false, full_kv);
                let capped =
                    run_2rocks_case(first, second, map_first, nf, ns, min_n, true, full_kv);
                let ok = full.surviving_mappings == capped.surviving_mappings
                    && full.skipped == capped.skipped
                    && capped.density_violations == 0
                    && full.kv == capped.kv;
                if !ok {
                    fail += 1;
                    println!(
                        "FAIL pair=({},{}) case={} min_n={} nf={} ns={} full_surv={} capped_surv={} full_skip={} capped_skip={} density_violations={} kv_equal={}",
                        i,
                        j,
                        ci + 1,
                        min_n,
                        nf,
                        ns,
                        full.surviving_mappings.len(),
                        capped.surviving_mappings.len(),
                        full.skipped,
                        capped.skipped,
                        capped.density_violations,
                        full.kv == capped.kv,
                    );
                }
            }
...
fn parse_blobs(v: &[u8], out: &mut Vec<Vec<u8>>) {
    validate_value_chain(v).expect("valid regular DB value chain");
    let mut pos = 0usize;
    while pos < v.len() {
...
fn merge_prefix_rebuild_matches() {
    ...
    for src in &sources {
        ...
        for item in db.iterator_opt(IteratorMode::From(&lower, Direction::Forward), ro) {
            ...
            assert_eq!(k.len(), 16, "regular DB key must be 16 bytes");
