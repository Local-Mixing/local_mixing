...
#[test]
fn length_prefixed_decoder_declines_tagged_values() {
    // A length-prefixed g57 decoder reads [len] and requires len % 3 == 0.
    // A tagged MPX1 value must yield zero circuits (first chunk len = 2),
    // rather than being misparsed as a g57 circuit.
    fn length_prefixed_walk(value: &[u8]) -> usize {
    ...
    assert_eq!(length_prefixed_walk(&tagged), 0);
