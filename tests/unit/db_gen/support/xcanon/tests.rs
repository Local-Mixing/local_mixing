...
#[test]
fn pure_g57_canonicalization_agrees_with_circuit_sequence() {
    let mut rng = Lcg(0x1234_5678_9abc_def0);
    for case in 0..20_000 {
        let len = 3 + rng.below(6) as usize;
        let g57 = random_g57_circuit(&mut rng, 10, len);
        let mut g57_circuit = CircuitSeq { gates: g57.clone() };
        g57_circuit.canonicalize();
        let want: Vec<XGate> = g57_circuit
            .gates
            .iter()
            .copied()
            .map(XGate::from_g57)
