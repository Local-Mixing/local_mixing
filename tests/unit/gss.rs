...
fn command_surface_parses() {
    let matches = command()
        .try_get_matches_from(["gss", "--dry-run", "--config", "custom.toml"])
        .unwrap();
...
fn recipe_manifest_is_seed_free_normalized_and_lifecycle_independent() {
    ...
    assert!(expected.starts_with("gss_command_recipe=7\n"));
...
fn manifest_comparison_accepts_exact_and_rejects_changed_or_oversized() {
    ...
    let desired = "gss_command_recipe=7\nn=128\n";
    fs::write(&path, desired).unwrap();
    compare_recipe_manifest(&path, desired).unwrap();

    let mismatch = compare_recipe_manifest(&path, "gss_command_recipe=7\nn=129\n").unwrap_err();
...
#[test]
fn pre_wrapper_adoption_is_rejected_without_changing_saved_artifacts() {
    let base = temp_path("retired-pre-wrapper");
    let run_dir = base.join("run");
    fs::create_dir_all(&run_dir).unwrap();
    ...
    fs::write(run_dir.join("gss.mpmct1"), "saved gadget\n").unwrap();
    let config_path = base.join("recipe.toml");
    let target_dir = base.join("target");
    for adopt in [false, true] {
        for calibration in [false, true] {
            ...
            assert!(
                error
                    .message
                    .contains("pre-wrapper run adoption has been retired"),
                "{error:?}"
            );
            assert!(!target_dir.exists());
            ...
            assert_eq!(
                fs::read_to_string(run_dir.join("gss.mpmct1")).unwrap(),
                "saved gadget\n"
            );
        }
    }
...
#[test]
fn managed_v7_dry_run_preserves_manifest_and_rejects_changed_binary() {
    let base = temp_path("managed-v7");
    let run_dir = base.join("run");
    let target_dir = base.join("target");
    ...
    let original_manifest = recipe_manifest(
        ...
        hash_file(&root.join("scripts/gss_mix.sh")).unwrap(),
    );
...
fn retired_fresh_modes_are_rejected_before_artifacts_or_builds() {
    ...
    for mode in ["product-2223", "2223", "nonlinear193"] {
        ...
        let error = run_inner(&matches).unwrap_err();
        assert!(
            error.message.contains("has been retired from GSS"),
            "{error:?}"
        );
        assert!(!run_dir.exists());
        assert!(!target_dir.exists());
...
#[test]
fn retired_markdown_recipe_reports_current_config_format() {
    let error = parse_config(
        "<!-- GSS_MIX_CONFIG_BEGIN -->\n```ini\nn = 4\n```\n<!-- GSS_MIX_CONFIG_END -->",
    )
    .unwrap_err();
    assert!(
        error.contains("Markdown GSS recipes have been retired"),
        "{error}"
    );
    assert!(error.contains("configs/gss.example.toml"), "{error}");
}
...
#[test]
fn retired_managed_recipes_fail_before_building_and_leave_saved_runs_unchanged() {
    for version in [3, 4, 5, 6] {
        let base = temp_path("retired-managed");
        let run_dir = base.join("run");
        let target_dir = base.join("target");
        fs::create_dir_all(&run_dir).unwrap();
        let recipe = format!(
        ...
        let config_path = base.join("recipe.toml");
        fs::write(&config_path, recipe).unwrap();
        let original_manifest = format!("gss_command_recipe={version}\nrecorded=unchanged\n");
        let manifest_path = run_dir.join("gss_command.conf");
        fs::write(&manifest_path, &original_manifest).unwrap();
        for dry_run in [false, true] {
            ...
            assert!(
                error.message.contains("versions 3–6 have been retired"),
                "{error:?}"
            );
            assert!(!target_dir.exists());
            assert_eq!(
...
#[test]
fn current_manifest_bytes_preserve_saved_run_identity() {
    let raw = parse_config("config_version = 1\nsource.wires = 4").unwrap();
    let config = resolve_for_test(&raw).unwrap();
    // Keep existing v7 runs resumable by preserving every field and its order.
    let expected = concat!(
        "gss_command_recipe=7\nn=4\nfrozen_db_dir=unset\nfrozen_curated_dir=unset\n",
        ...
        "script_gss_mix_xxh3=00000000000000000000000000000001\n",
    );
