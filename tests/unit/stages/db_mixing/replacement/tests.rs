...
fn stable_mode_picks_only_near_size() {
    ...
    // Stable uses the strict curated-first selection rule: a curated
    // far-size candidate hides the regular same-size one, producing a miss.
...
// Store one 1-gate g57 friend for `g57_circuit`'s key, in the builder's canonical
// wire space, and return (key, value).
fn store_friend(g57_circuit: &CircuitSeq) -> ([u8; 16], Vec<u8>) {
    let (polys, order, _used) = g57_circuit.canonicalize_polys_single(false);
    let key = xxh3_128(&polys_repr_blob(&polys)).to_le_bytes();
    let mut stored = g57_circuit.clone();
...
fn compressing_returns_equivalent_shorter_friend() {
    ...
    let (key, value) = store_friend(&g57_circuit);
...
// The degree filter reads the exact degree from the ANF, so its decision
// must be independent of the RNG seed. Run one over-degree window across
// many seeds and require the same answer every time.
#[test]
fn degree_filter_is_exact_and_seed_independent() {
    ...
    // The forward direction is over the cap, but this window's reverse
    // has a lower degree and can still be canonicalized and looked up.
    // Each direction is bounded independently, so at most one key per run
    // may reach the store.
    assert!(
        looked_up <= 64,
...
fn regular_fallback_false_suppresses_the_regular_stage() {
    ...
    let comp = db_replace_with(
        ...
        DbMode::Compressing,
        DegreeGuard::OFF,
        // Apply the mode policy used by `db_replace`. Its contract is
...
// The curated-first cascade: expansion with curated armed probes curated
...
fn compressing_rejects_growth_but_size_agnostic_accepts_it() {
    ...
    // g57_circuit computes the same function as `window` (the [3,4,5] pair cancels).
