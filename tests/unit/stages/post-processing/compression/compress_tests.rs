...
fn two_member_complemented_pair_collapses() {
    ...
    // complement into the parity slot. Two-gate groups must fold too.
...
fn downhill_collapses_crossing_ladder() {
    ...
    // gathering. Gather-only processing plus downhill alone still wins.
    let (out2, _) = compress(gates.clone(), 4, &gather_only());
...
// Gather-only parameters disable transport, separation passes, reverse
// passes and downhill rewriting.
fn gather_only() -> CompressParams {
    CompressParams {
        downhill: false,
        transport: false,
        sep_reads: false,
...
fn toffoli_sliding_pair_cancels() {
    // A control flipped under the other controls slides through with a
    // polarity change, so the two copies are one identity. Gather-only processing
    ...
    assert!(equal_on(&gates, &out, 4, None));
    let (gathered, _) = compress(gates.clone(), 4, &gather_only());
    assert_eq!(
        gathered.len(),
        3,
        "gather-only processing is blocked by the flip"
    );
}
