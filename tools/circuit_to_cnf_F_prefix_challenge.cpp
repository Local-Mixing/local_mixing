...
int main(int argc, char **argv) {
  if (argc != 4) {
    std::cerr << "usage: circuit_to_cnf_F_prefix_challenge F.txt out.cnf low64\n";
    return 2;
  }

  auto gates = parse(argv[1]);
  uint64_t low = parse_u64(argv[3]);

  std::vector<int> state(128);
  for (int i = 0; i < 128; i++) state[i] = i + 1;
  int next_var = 129;
  std::vector<std::vector<int>> clauses;
  clauses.reserve(6 * gates.size() + 178);

  // Output is 0^64 || low64, with little-endian bit numbering.
  for (int i = 0; i < 64; i++) {
    clauses.push_back({((low >> i) & 1ULL) ? state[i] : -state[i]});
  }
  for (int i = 64; i < 128; i++) clauses.push_back({-state[i]});

  // Reverse the circuit so `state` names input bits.
  ...
  // Input has 50 leading zero bits.
