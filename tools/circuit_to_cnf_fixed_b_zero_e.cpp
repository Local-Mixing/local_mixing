...
int main(int argc, char **argv) {
  if (argc != 4) {
    std::cerr << "usage: circuit_to_cnf_fixed_b_zero_e F.txt out.cnf b128\n";
    return 2;
  }

  auto gates = parse(argv[1]);
  auto b_value = parse_hex128(argv[3]);
  ...
  // Input B is the requested value; input C/Z is fixed to zero for a concrete witness.
  for (int i = 0; i < 128; i++) {
    clauses.push_back({b_value[i] ? state[128 + i] : -state[128 + i]});
  }
  ...
  // Middle output E is zero.
  for (int i = 0; i < 128; i++) clauses.push_back({-state[128 + i]});
