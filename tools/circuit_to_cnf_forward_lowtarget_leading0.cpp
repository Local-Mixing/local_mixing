...
int main(int argc, char **argv) {
  if (argc != 6 && argc != 7) {
    std::cerr << "usage: circuit_to_cnf_forward_lowtarget_leading0 F.txt out.cnf n target64 leading_zero_bits [target_bits]\n";
    return 2;
  }
  ...
  int target_bits = n < 64 ? n : 64;
  if (argc == 7) target_bits = parse_int(argv[6], "target bit count");
  if (n <= 0 || leading_zero_bits > n || target_bits < 0 || target_bits > n ||
      target_bits > 64) {
  ...
  clauses.reserve(6 * gates.size() + leading_zero_bits + target_bits);
